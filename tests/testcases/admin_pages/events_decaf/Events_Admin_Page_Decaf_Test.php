...
class Events_Admin_Page_Decaf_Test extends EE_UnitTestCase
{
     ...
     * @since 4.6
     */
    public function test_default_tickets_update_with_no_ticket_dates()
    {
        ...
        // TKT_start is set in the decaf ticket saves without seconds,
        // so we need to set the seconds to zero as well for comparison.
        $this->_default_dates['TKT_start']->setTime(
            $this->_default_dates['TKT_start']->format('H'),
            $this->_default_dates['TKT_start']->format('i')
        );
        /**
         * tkt end date is the same as start date because
         * here's the code execution in Events_Admin_page::default_tickets_update:
         *  1. If no TKT_end_date, then set to DTT_EVT_start.
         *  2. However, this means that DTT_EVT_start (because its static)
         *     will always be earlier than what the value for TKT_start_date is.
         *  3. So later the code changes TKT_end_date to be the SAME as TKT_start_date
         *     and then sets it forward by a day.  So that's what our expected value will be.
         */
    ...
    protected function _testing_updates($context_for_error_messages, $data_to_replace = [])
    {
        ...
        foreach ($formats_to_test['date'] as $date_format) {
            foreach ($formats_to_test['time'] as $time_format) {
                ...
                if (! empty($data_to_replace)) {
                    ...
                    //     if (is_array($second_level) /*&& ! empty($second_level)*/) {
                    //         foreach ($second_level as $third_level => $fourth_level) {
                    //             if (is_array($fourth_level)/* && ! empty($fourth_level)*/) {
        ...
        $new_event = EEM_Event::instance('America/Vancouver')->refresh_entity_map_from_db($this->_event->ID());
        ...
        foreach ($evt_dtts as $edtt) {
            ...
            $this->assertArrayHasKey($edtt->ID(), $formats_for_compare);
            $formats = isset($formats_for_compare[ $edtt->ID() ]) ? $formats_for_compare[ $edtt->ID() ] : ['', ''];
            ...
            $this->assertInstanceof('EE_Ticket', $evt_tkt);
            $this->assertDateWithinOneMinute(
