...
class EEH_Line_Item_Test extends EE_UnitTestCase
{
    ...
    public function test_get_items_subtotal()
    {
        ...
        $items_subtotals          = EEM_Line_Item::instance()->get_all_of_type_for_transaction(
    ...
    public function test_calculate_reg_final_prices_per_line_item__3_taxable_tickets_with_a_discount()
    {
        ...
        /** @var EE_Ticket $new_tkt */
        $new_tkt = $this->new_model_obj_with_dependencies('Ticket', ['TKT_price' => 6]);
        ...
        // honestly the easiest way to confirm the total was right is to visualize the tree
        // EEH_Debug_Tools::printr($totals, '$totals', __FILE__, __LINE__);
        // EEH_Line_Item::visualize( $transaction->total_line_item() );
        ...
        $ten_dollar_ticket = EEM_Line_Item::instance()->get_one(
            [
                [
                    'TXN_ID'         => $transaction->ID(),
                    'LIN_unit_price' => 10,
        ...
        $this->assertEqualsWithDelta(
            10.30,
            $totals[ $ten_dollar_ticket->ID() ],
            EE_UnitTestCase::FLOATING_POINT_DELTA
        );
        //one ticket should be 20 pre-tax
        $twenty_dollar_ticket = EEM_Line_Item::instance()->get_one(
            [
                [
                    'TXN_ID'         => $transaction->ID(),
                    'LIN_unit_price' => 20,
        ...
        $this->assertEqualsWithDelta(
            20.60,
            $totals[ $twenty_dollar_ticket->ID() ],
            EE_UnitTestCase::FLOATING_POINT_DELTA
        );
        //one ticket should be for 6 pre-tax (although its non-taxable anyway)
        $six_dollar_ticket = EEM_Line_Item::instance()->get_one(
            [
                [
                    'TXN_ID'         => $transaction->ID(),
                    'LIN_unit_price' => 6,
    ...
    {
        $number_of_tickets = 2;
        $transaction       = $this->new_typical_transaction(['ticket_types' => $number_of_tickets]);
        //add another ticket purchase for one of the same events
        $event1 = EEM_Event::instance()->get_one(
            [
                [
     ...
     * So we should end up with one costing $5.50 and one $5.
     *
     * @group 8193
     * @throws EE_Error
     * @throws ReflectionException
    ...
    function test_calculate_reg_final_prices_per_line_item__percent_discount_partially_taxable()
    {
        ...
        $grand_total = $this->new_model_obj_with_dependencies(
            'Line_Item',
            [
                'LIN_name'  => 'total',
        ...
        $subtotal    = $this->new_model_obj_with_dependencies(
            'Line_Item',
            [
                'LIN_name'       => 'subtotal',
    ...
    function test_calculate_reg_final_prices_per_line_item__discount_only_for_one_event_subtotal()
    {
        ...
        $subtotal_a  = $this->new_model_obj_with_dependencies(
            'Line_Item',
            [
                'LIN_name'       => 'subtotal_a',
                'LIN_type'       => EEM_Line_Item::type_sub_total,
        ...
        $subtotal_b         = $this->new_model_obj_with_dependencies(
            'Line_Item',
            [
                'LIN_name'       => 'subtotal_b',
                'LIN_type'       => EEM_Line_Item::type_sub_total,
        ...
        $ticket_line_item_a = $this->new_model_obj_with_dependencies(
            'Line_Item',
            [
        ...
        $ticket_line_item_b = $this->new_model_obj_with_dependencies(
            'Line_Item',
            [
    ...
    function test_cancel_ticket_line_item__with_sub_items_already()
    {
        ...
        $cancellation_line_items    =
            EEH_Line_Item::get_descendants_of_type($event_subtotal, EEM_Line_Item::type_cancellation);
    ...
    {
        $t = $this->new_typical_transaction(['taxable_tickets' => 0]);
    ...
    function test_recalculate_total_including_taxes_after_ticket_cancellation()
    {
        ...
        $transaction   = $this->new_typical_transaction(['taxable_tickets' => 1]);
        ...
        $ticket2 = $this->new_ticket(['TKT_price' => 15, 'TKT_taxable' => false]);
        ...
        foreach ($ticket_line_items as $ticket_line_item) {
            if (
                ...
                && (string) $ticket_line_item->OBJ_ID() === (string) $ticket2->ID()
            ) {
    ...
    {
        $transaction = $this->new_typical_transaction(['ticket_types' => 2]);
        $total_line_item = $transaction->total_line_item();
        ...
        foreach (EEH_Line_Item::get_event_subtotals($total_line_item) as $event_line_item) {
            ...
            $this->assertEquals(
                $event_line_item->unit_price(),
                ($event_line_item->total() / $event_line_item->quantity())
            );
        }
    }
