	...
	public function test_sum_tickets_currently_available_at_datetime() {
		...
		foreach ( $scenarios as $scenario ) {
			if ( $scenario->get_expected( 'sum_tickets_currently_available_at_datetime' ) !== false ) {
				$actual_returned = EEM_Ticket::instance()->sum_tickets_currently_available_at_datetime( $scenario->get_scenario_object()->ID() );
				$this->assertEquals( $scenario->get_expected( 'sum_tickets_currently_available_at_datetime' ), $actual_returned );
			}
		}
	}
