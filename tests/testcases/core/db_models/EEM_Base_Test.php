...
//	}
    ...
    /**
     * This tests when there is a deletion of an entity that is related to another via a join table, that the join table
     * relationship is also permanently deleted.
     * @group test_joins
     */
    public function test_delete_with_join_table()
    {
        /** @var EE_Message_Template_Group $message_template_group */
        $message_template_group = EE_Message_Template_Group::new_instance();
        $message_template_group->save();
        /** @var EE_Event $event_joined_with */
        $event_joined_with = $this->factory->event->create();
        $message_template_group->_add_relation_to($event_joined_with, 'Event');
        $message_template_group->save();
        /** @var EE_Registration $registration_joined_with_event */
        ...
        $event_message_template = EEM_Event_Message_Template::instance()->get_one(array(
            array(
                'EVT_ID' => $event_joined_with->ID(),
                'GRP_ID' => $message_template_group->ID()
            )
        ));
        $this->assertInstanceOf('EE_Event_Message_Template', $event_message_template);
        $success = $event_joined_with->delete_permanently();
        //expect 0 because of the related delete block
        $this->assertEquals(0, $success);
        //reset error notices to prevent fail because of the notice
        EE_Error::reset_notices();
        //join table should still exist because of the blocked delete.
        $join_table_entity = EEM_Event_Message_Template::instance()->get_one_by_ID($event_message_template->ID());
        $this->assertInstanceOf('EE_Event_Message_Template', $join_table_entity);

        //Registration is a join table to attendee.  It should also be present because of the blocked delete.
        ...
        //try again but this time we'll explicitly instruct EEM_Base to ignore blocks.
        $success = EEM_Event::instance()->delete_permanently(
            array(
                0 => array('EVT_ID' => $event_joined_with->ID())
            ),
            false
        );

        //this time we do expect a row was deleted. Since Event is a CPT we expect 2 rows deleted.
        $this->assertEquals(2, $success);

        //assert the join table entry no longer exists in the db
        $join_table_entity = EEM_Event_Message_Template::instance()->get_one_by_ID($event_message_template->ID());
