...
class EE_Payment_Processor_Test extends EE_UnitTestCase
{
    ...
    protected function _new_typical_transaction()
    {
        ...
        $transaction  = $this->new_model_obj_with_dependencies('Transaction', ['TXN_total' => 10.00]);
    ...
    {
        return $this->new_model_obj_with_dependencies('Ticket', ['TKT_price' => $TKT_price]);
    }
    ...
    {
        return $this->new_model_obj_with_dependencies('Event', ['EVT_wp_user' => $EVT_wp_user->ID]);
    }
    ...
    public function test_process_payment__onsite__success()
    {
        ...
        $pm           = $this->new_model_obj_with_dependencies('Payment_Method', ['PMD_type' => 'Mock_Onsite']);
        ...
        add_action(
            'AHEE__EE_Payment_Processor__update_txn_based_on_payment',
            [$this, 'assert_payment_message_trigger_for_success'],
            10,
            2
        );
    ...
    public function test_process_payment__offsite__declined_then_approved()
    {
        ...
        /** @type EE_Payment_Method $pm */
        $pm                         =
            $this->new_model_obj_with_dependencies('Payment_Method', ['PMD_type' => 'Mock_Offsite']);
        $transaction                = $this->_new_typical_transaction();
        $successful_payment_actions = EEH_Array::is_set(
            $wp_actions,
            'AHEE__EE_Payment_Processor__update_txn_based_on_payment__successful',
        ...
        $payment = $payment_processor->process_ipn(
            ['status' => EEM_Payment::status_id_pending, 'gateway_txn_id' => $payment->txn_id_chq_nmbr()],
            $transaction,
            $pm
        );
        $this->assertEquals(EEM_Payment::status_id_pending, $payment->status());
        ...
        //APPROVED IPN
        $payment = $payment_processor->process_ipn(
            ['status' => EEM_Payment::status_id_approved, 'gateway_txn_id' => $payment->txn_id_chq_nmbr()],
            $transaction,
            $pm
        );
    ...
    {
        /** @type EE_Payment_Method $pm */
        $pm          = $this->new_model_obj_with_dependencies('Payment_Method', ['PMD_type' => 'Admin_Only']);
        $transaction = $this->_new_typical_transaction();
        global $wp_actions;
        $successful_payment_actions = EEH_Array::is_set(
            $wp_actions,
        ...
        $payment_processor = EE_Registry::instance()->load_core('Payment_Processor');
        $payment           =
            $payment_processor->process_payment($pm, $transaction, null, null, 'success', 'CART', true, true);
    ...
    {
        /** @type EE_Payment_Method $pm */
        $pm = $this->new_model_obj_with_dependencies('Payment_Method', ['PMD_type' => 'Mock_Offsite']);
