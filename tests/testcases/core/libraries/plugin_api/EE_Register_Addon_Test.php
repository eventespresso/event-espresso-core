...
class EE_Register_Addon_Test extends EE_UnitTestCase
{
    ...
    {
        if (EE_Registry::instance()->addons->has($this->_addon_name)) {
            $addon = EE_Registry::instance()->addons->get($this->_addon_name);
            $main_file_path_before_deregistration = $addon->get_main_plugin_file_basename();
            EE_Register_Addon::deregister($this->_addon_name);
            $this->assertArrayNotHasKey($this->_addon_name, EE_Registry::instance()->addons);
    ...
    private function _pretend_capabilities_registered()
    {
        ...
        $expected_caps = ['edit_thing', 'edit_things', 'edit_others_things', 'edit_private_things'];
        $this->assertArrayContains($expected_caps, $role_caps);
    }
    ...
    //		$this->assertAttributeNotEmpty($this->_addon_name,EE_Registry::instance()->addons);
    //
    ...
    public function test_register_mock_addon_success()
    {
        ...
        require_once(EE_MOCKS_DIR . 'addons/eea-new-addon/eea-new-addon.php');
        require_once(EE_MOCKS_DIR . 'addons/eea-new-addon/EE_New_Addon.class.php');
        EE_New_Addon::register_addon();
    ...
    //			$this->assertTrue( FALSE );
    //		}
    //		$this->assertFalse(property_exists(EE_Registry::instance()->addons, $this->_addon_name));
    //
    //
    ...
    public function test_register_addon_called_twice_on_activation()
    {
        ...
        $this->assertFalse(property_exists(EE_Registry::instance()->addons, $this->_addon_name));
        EE_Register_Addon::register($this->_addon_name, $this->_reg_args);
        $this->assertTrue(EE_Registry::instance()->addons->has($this->_addon_name));
        $this->assertInstanceOf(
            $this->_addon_name,
            EE_Registry::instance()->addons->get($this->_addon_name)
        );
        global $wp_actions;
