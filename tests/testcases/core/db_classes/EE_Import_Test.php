...
class EE_Import_Test extends EE_UnitTestCase
{
    ...
    protected function _assertNoImportErrors()
    {
        ...
        $this->assertEmpty(
            EE_Import::instance()->get_total_update_errors(),
            $notices['errors'] ?? ''
        );
        $this->assertEmpty(
            EE_Import::instance()->get_total_insert_errors(),
            $notices['errors'] ?? ''
        );
    }
    ...
    public function test_save_data_array_to_db__from_other_site__data_with_same_ids()
    {
        ...
        //ok give it a whirl...
        $new_mappings = EE_Import::instance()->save_data_rows_to_db($csv_data_rows, true, []);
        ...
        //the newly inserted datetime should have bene associated to the new event for $other_db_event_props
        ...
        $inserted_datetime_ticket_from_other_db = EEM_Datetime_Ticket::instance()->get_one_by_ID(
            $datetime_ticket_mapping
        );
    ...
    public function test_save_data_array_to_db__from_other_site__avoid_inserting_conflicting_data()
    {
        $term_taxonomy = $this->new_model_obj_with_dependencies(
            'Term_Taxonomy',
            ['taxonomy' => 'category', 'description' => 'original term-taxonomy']
        );
        ...
        EE_Import::instance()->save_data_rows_to_db(
            $csv_data_rows, true,
            [
                'Term' => [
                    $term_taxonomy->get('term_id') => $term_taxonomy->get(
                        'term_id'
                    ),
                ],
            ]
    ...
    public function test_save_data_array_to_db__from_other_site__fks_that_dont_exist()
    {
        ...
        $att                 = $this->new_model_obj_with_dependencies(
            'Attendee',
            ['STA_ID' => 99999, 'CNT_ISO' => '77'],
            false
        );
        $att_props           = $att->model_field_array();
        $att_props['ATT_ID'] = 1234;

        $csv_data  = ['Attendee' => [$att_props]];
        $att_count = EEM_Attendee::instance()->count();
        $mappings  = EE_Import::instance()->save_data_rows_to_db($csv_data, true, []);

        $this->assertIsArray($mappings);
        $this->assertEquals($att_count + 1, EEM_Attendee::instance()->count());

        //the STA_ID should have been set to 0, but the CNT_ISO should have been left as-is
        $att_id_in_db = $mappings['Attendee'][1234];
    ...
    public function test_save_data_array_to_db__from_other_site__fks_that_point_to_multiple_models()
    {
        ...
        $inserted_term_r_id =
            $new_mappings['Term_Relationship'][ EEM_Term_Relationship::instance()->get_index_primary_key_string(
                $csv_data['Term_Relationship'][0]
            ) ];
    ...
    function test_save_data_array_to_db__from_other_site__no_duplicate_term_relationships()
    {
        ...
        $this->markTestSkipped(
            'fixed on ticket 7537, https://events.codebasehq.com/projects/event-espresso/tickets/7537'
        );
        ...
        $a_real_term_r        = $this->new_model_obj_with_dependencies(
            'Term_Relationship',
            [
                'object_id'        => $a_real_event->ID(),
                'term_taxonomy_id' => $a_real_term_taxonomy->ID(),
                'term_order'       => $new_term_r_order,
            ]
        );
     ...
     * @throws EE_Error
     * @throws ReflectionException
     * @todo  : account for wp 4.2 term splitting
    ...
    function test_save_data_array_to_db__from_this_site__term_split()
    {
        ...
        $term  = $this->new_model_obj_with_dependencies('Term', ['name' => 'Jaguar', 'slug' => 'jag']);
        $ttcar = $this->new_model_obj_with_dependencies(
            'Term_Taxonomy',
            ['term_id' => $term->ID(), 'taxonomy' => 'cars', 'description' => 'A fast car']
        );
        $ttcat = $this->new_model_obj_with_dependencies(
            'Term_Taxonomy',
            ['term_id' => $term->ID(), 'taxonomy' => 'cats', 'description' => 'A large black cat that likes to swim']
        );
        ...
        //import it
        $new_mapping = EE_Import::instance()->save_data_rows_to_db($csv_data, false, []);
    ...
    function test_save_data_array_to_db__from_this_site__term_split__in_wp_41()
    {
        ...
        EE_Import::instance()->save_data_rows_to_db($csv_data, false, []);
...
if (! function_exists('_split_shared_term')) {
     ...
     *
     */
    function _split_shared_term(int $term_id, int $term_taxonomy_id)
    {
        global $wpdb;
        ...
        $term_taxonomy   = $wpdb->get_row(
            $wpdb->prepare("SELECT * FROM $wpdb->term_taxonomy WHERE term_taxonomy_id = %d", $term_taxonomy_id)
        );
        $children_tt_ids = $wpdb->get_col(
            $wpdb->prepare(
                "SELECT term_taxonomy_id FROM $wpdb->term_taxonomy WHERE taxonomy = %s AND parent = %d",
        ...
        $shared_term_taxonomies =
            $wpdb->get_row($wpdb->prepare("SELECT taxonomy FROM $wpdb->term_taxonomy WHERE term_id = %d", $term_id));
...
if (! function_exists('wp_get_split_terms')) {
     ...
     * @return int
     */
    function wp_get_split_terms(int $old_term_id)
    {
...
if (! function_exists('wp_get_split_term')) {
     ...
     *
     */
    function wp_get_split_term(int $old_term_id, string $taxonomy)
    {
