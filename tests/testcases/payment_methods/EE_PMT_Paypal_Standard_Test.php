...
class EE_PMT_Paypal_Standard_Test extends EE_UnitTestCase
{
    ...
    public function test_handle_payment_update__refund()
    {
        ...
        $ppm = $this->new_model_obj_with_dependencies(
            'Payment_Method',
            ['PMD_type' => 'Paypal_Standard']
        );
        /** @type EEG_Paypal_Standard $ppg */
        $ppg = $ppm->type_obj()->get_gateway();
        $ppg->set_settings(['paypal_id' => $this->_paypal_id]);
        ...
        $this->assertEquals($txn_total, $p->amount());
    }
    ...
    public function test_handle_payment_update__paypal_adds_taxes_and_shipping()
    {
        ...
        $this->assertNotEquals(
            $old_tax_total,
            $t->tax_total(),
            'Its not necessarily wrong for the old tax to match the new tax; but if they match we can\'t be very sure the tax total was updated'
        );
    ...
    public function test_update_txn_based_on_payment__paypal_adds_taxes_and_shipping__twice()
    {
        ...
        $this->assertNotEquals(
            $tax_in_1st_ipn,
            $old_tax_total,
            'Its not necessarily wrong for the old tax to match the new tax; but if they match we can\'t be very sure the tax total wasnt updated'
        );
        ...
        $p2              = $this->new_model_obj_with_dependencies(
            'Payment',
            [
                'TXN_ID'      => $t->ID(),
                'STS_ID'      => EEM_Payment::status_id_approved,
    ...
    public function test_set_redirection_info__success()
    {
        ...
        $p =
            $this->new_model_obj_with_dependencies(
                'Payment',
                ['TXN_ID' => $t->ID(), 'PMD_ID' => $ppm->ID(), 'PAY_amount' => $t->total()]
            );
        ...
        $this->assertEquals(
            sprintf('%s for %s', $first_item->ticket()->name(), $first_item->ticket_event_name()),
            $rargs['item_name_1']
        );
        $this->assertEquals($first_item->ticket()->price(), $rargs['amount_1']);
        $this->assertEquals(
            sprintf('%s for %s', $second_item->ticket()->name(), $second_item->ticket_event_name()),
            $rargs['item_name_2']
        );
    ...
    public function test_set_redirection_info__with_paypal_taxes_and_shipping()
    {
        ...
        EEH_Line_Item::set_total_tax_to(
            $t->total_line_item(),
            4,
            'paypal taxes',
            'paypal did thi',
    ...
    public function test_set_redirect_info__with_promotion()
    {
        ...
        $ppg->set_settings($this->_test_settings);
        $t           = $this->new_typical_transaction(['ticket_types' => 2]);
        $event       = EEM_Event::instance()->get_one([['Registration.TXN_ID' => $t->ID()]]);
