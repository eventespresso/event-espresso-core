...
function espresso_display_authnet_sim() {
	global $EE_Session;
	$gateway = 'authnet_sim';
	$gateway_data = $EE_Session->get_session_data(FALSE, 'gateway_data');

	$authnet_sim_settings = $gateway_data['payment_settings'][$gateway];
...
?>
			<a id="payment-gateway-button-<?php echo $gateway;?>" class="reg-page-payment-option-lnk<?php echo $gateways[ $gateway ]['css_link_class'];?>" rel="<?php echo $gateway;?>" href="<?php echo $gateways[ $gateway ]['form_url'];?>" >
				<img src="<?php echo $authnet_sim_settings['button_url'];?>" alt="Pay using Authorize.net" />
			</a>
...
add_action('action_hook_espresso_display_offsite_payment_gateway', 'espresso_display_authnet_sim');