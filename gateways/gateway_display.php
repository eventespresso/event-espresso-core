...
//	printr($active_gateways);die();
//	echo var_dump($payment_settings);

	foreach ($gateway_data['active_gateways'] as $gateway => $path) {
		if (file_exists($path . "/payment.php")) {
			require_once($path . "/payment.php");
		} elseif (file_exists(EVENT_ESPRESSO_PLUGINFULLPATH . 'gateways/' . $gateway . '/payment.php')) {
			$gateway_data['active_gateways'][$gateway] = EVENT_ESPRESSO_PLUGINFULLPATH . 'gateways/' . $gateway;
			update_user_meta($espresso_wp_user, 'active_gateways', $gateway_data['active_gateways']);
			require_once(EVENT_ESPRESSO_PLUGINFULLPATH . 'gateways/' . $gateway . '/payment.php');
		} else {
			echo '<div id="file_missing_error"><h2>payment.php is missing for gateway ' . $gateway;
			echo '. The site admin should deacivate ' . $gateway . ' and try to reactivate.</h2></div>';
		}
	}
