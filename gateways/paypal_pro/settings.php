...
function event_espresso_paypal_pro_payment_settings() {
	...
	if ($payment_settings['paypal_pro']['active'] == false || isset($_REQUEST['deactivate_paypal_pro']) && $_REQUEST['deactivate_paypal_pro'] == 'true' ){
		...
		<div class="inside">
			<div class="padding"> 
				...
				<?php
					...
					echo '<ul>';
					if (!isset($payment_settings['paypal_pro']['active'])){
						echo '<li style="width:50%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_paypal_pro=true#paypal_pro\';" class="yellow_alert pointer"><strong>' . __('The PayPal Pro Gateway is installed. Would you like to activate it?','event_espresso') . '</strong></li>';
					}else{
						switch ($payment_settings['paypal_pro']['active']){
						
							case false:
								echo '<li>PayPal Pro Gateway is Installed.</li>';
								echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&reactivate_paypal_pro=true#paypal_pro\';" class="green_alert pointer"><strong>' . __('Activate PayPal Pro Gateway?', 'event_espresso') . '</strong></li>';
								break;
...
function event_espresso_display_paypal_pro_settings() {
	...
	?>
<form method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>#paypal_pro">
	<table class="form-table">
		<tbody>
			<tr>
				...
				<td><input class="regular-text" type="text" name="email" id="paypal_pro_email" size="35" value="<?php echo empty($payment_settings['paypal_pro']['email']) ? '' : $payment_settings['paypal_pro']['email']; ?>"></td>
			</tr>
			<tr>
				<th><label for="paypal_pro_username">
						<?php _e('Paypal API Username', 'event_espresso'); ?>
					</label></th>
				<td><input class="regular-text" type="text" name="username" id="paypal_pro_username" size="35" value="<?php echo empty($payment_settings['paypal_pro']['username']) ? '' : $payment_settings['paypal_pro']['username']; ?>"></td>
			</tr>
			<tr>
				<th><label for="paypal_pro_password">
						<?php _e('Paypal API Password', 'event_espresso'); ?>
					</label></th>
				<td><input class="regular-text" type="text" name="password" id="paypal_pro_password" size="35" value="<?php echo empty($payment_settings['paypal_pro']['password']) ? '' : $payment_settings['paypal_pro']['password']; ?>"></td>
			</tr>
			<tr>
				<th><label for="paypal_pro_signature">
						<?php _e('Paypal API Signature', 'event_espresso'); ?>
					</label></th>
				<td><input class="regular-text" type="text" name="signature" id="paypal_pro_signature" size="35" value="<?php echo empty($payment_settings['paypal_pro']['signature']) ? '' : $payment_settings['paypal_pro']['signature']; ?>"></td>
			</tr>
			<tr>
				<th><label for="paypal_pro_currency_format">
						<?php _e('Country Currency', 'event_espresso'); ?>
						<?php apply_filters('espresso_help', 'paypal_pro_currency_info') ?>
					</label></th>
				<td><select name="currency_format" data-placeholder="Choose a currency..." class="chzn-select wide">
						<option value="<?php echo empty($payment_settings['paypal_pro']['currency_format']) ? 'USD' : $payment_settings['paypal_pro']['currency_format']; ?>"><?php echo $payment_settings['paypal_pro']['currency_format']; ?></option>
						<option value="USD">
			...
			<tr>
				...
				<td><?php
						...
						$credit_cards = explode(",", empty($payment_settings['paypal_pro']['credit_cards']) ? '' : $payment_settings['paypal_pro']['credit_cards']);
					?>
			...
			<tr>
				...
				<td><?php echo select_input('use_sandbox', $values, empty($payment_settings['paypal_pro']['use_sandbox']) ? 'N' : $payment_settings['paypal_pro']['use_sandbox']);?>
					<br />
