 ...
 * @global  $wp_json_basic_auth_received_data    boolean indicating whether basic auth data was received (either in the
 *          normal HTTP header, or using the CGI-htaccess workaround, or the query string
 * @global  $wp_json_basic_auth_error            WP_Error for using when displaying errors
 * @global  $wp_json_basic_auth_success          boolean indicating we successfully authenticated the user using basic
 *          auth (false if we found basic auth data but it didn't correspond to a real user's credentials)
 */
