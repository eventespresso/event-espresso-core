...
class EE_Error extends Exception
{
    ...
    {
        $html =
            "<table cellpadding='5'><thead bgcolor='#f8f8f8'><th>Item</th><th align='left'>Details</th></thead><tbody>";
    ...
    {
        $has_error =
            isset(self::$_espresso_notices[$type_to_check]) && ! empty(self::$_espresso_notices[$type_to_check])
                ? true
                : false;
...
<div id="ee-error-message" class="error">';
	...
	<p>';
        ...
        foreach (self::$_all_exceptions as $time => $ex) {
            ...
            if (empty($ex['trace'])) {
                $trace_details .= __(
                    'Sorry, but no trace information was available for this exception.', 'event_espresso'
                );
            } else {
                $trace_details .= '
			...
			<table width="100%" border="0" cellpadding="5" cellspacing="0">
                ...
                foreach ($sorted_trace as $nmbr => $trace) {
                    ...
                    $args = isset($trace['args'])
                        ? $this->_convert_args_to_string($trace['args'])
                        : '';
                    ...
                    $nmbr_dsply = ! empty($nmbr)
                        ? $nmbr
                        : '&nbsp;';
                    $line_dsply = ! empty($line)
                        ? $line
                        : '&nbsp;';
                    $file_dsply = ! empty($file)
                        ? $file
                        : '&nbsp;';
                    $class_dsply = ! empty($class)
                        ? $class
                        : '&nbsp;';
                    $type_dsply = ! empty($type)
                        ? $type
                        : '&nbsp;';
                    $function_dsply = ! empty($function)
                        ? $function
                        : '&nbsp;';
                    $args_dsply = ! empty($args)
                        ? '( ' . $args . ' )'
                        : '';
                    $trace_details .= '
					<tr>
            ...
            foreach ($notices as $type => $notice) {
                if (is_array($notice) && ! empty($notice)) {
                    ...
                    self::$_espresso_notices[$type] =
                        is_array(self::$_espresso_notices[$type]) && ! empty(self::$_espresso_notices[$type])
                            ? self::$_espresso_notices[$type]
                            : array();
        ...
        if (! empty($pan_name)) {
            ...
            if (is_array($persistent_admin_notices) && isset($persistent_admin_notices[$pan_name])) {
                ...
                if (update_option('ee_pers_admin_notices', $persistent_admin_notices) === false) {
                    EE_Error::add_error(
                        sprintf(
                            __('The persistent admin notice for "%s" could not be deleted.', 'event_espresso'),
                            $pan_name
    ...
    public static function get_persistent_admin_notices($return_url = '')
    {
        ...
        if ($persistent_admin_notices) {
            // load scripts
            wp_register_script(
                'espresso_core', EE_GLOBAL_ASSETS_URL . 'scripts/espresso_core.js', array('jquery'),
                EVENT_ESPRESSO_VERSION, true
            );
            wp_register_script(
                'ee_error_js', EE_GLOBAL_ASSETS_URL . 'scripts/EE_Error.js', array('espresso_core'),
                EVENT_ESPRESSO_VERSION, true
            );
...
';
    ...
    public static function generate_error_code($file = '', $func = '', $line = '')
    {
        ...
        $error_code = ! empty($file[0])
            ? $file[0]
            : '';
        $error_code .= ! empty($func)
            ? ' - ' . $func
            : '';
    ...
    public function write_to_error_log($time = false, $ex = false, $clear = false)
    {
        ...
        try {
            ...
            if (! $clear) {
                ...
                $exception_log =
                    EEH_File::get_file_contents(EVENT_ESPRESSO_UPLOAD_DIR . 'logs' . DS . self::$_exception_log_file)
                    . $exception_log;
            }
            EEH_File::write_to_file(
                EVENT_ESPRESSO_UPLOAD_DIR . 'logs' . DS . self::$_exception_log_file, $exception_log
            );
        } catch (EE_Error $e) {
            EE_Error::add_error(
                sprintf(
...
function espresso_error_enqueue_scripts()
{
    ...
    wp_register_script(
        'espresso_core', EE_GLOBAL_ASSETS_URL . 'scripts/espresso_core.js', array('jquery'), EVENT_ESPRESSO_VERSION,
        false
    );
    wp_register_script(
        'ee_error_js', EE_GLOBAL_ASSETS_URL . 'scripts/EE_Error.js', array('espresso_core'), EVENT_ESPRESSO_VERSION,
        false
    );
}
