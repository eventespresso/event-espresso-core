...
class EE_Session
{
    ...
    public function transaction()
    {
        return isset($this->_session_data['transaction'])
               && $this->_session_data['transaction']
                  instanceof
                  EE_Transaction ? $this->_session_data['transaction']
            : null;
    }
        ...
        if ($session_data) {
            if (apply_filters('FHEE__EE_Session___perform_session_id_hash_check', WP_DEBUG)) {
                ...
                if ($hash_check && $hash_check !== md5($session_data)) {
                    EE_Error::add_error(
                        sprintf(
                            __('The stored data for session %1$s failed to pass a hash check and therefore appears to be invalid.',
                                'event_espresso'),
    ...
    public function update($new_session = false)
    {
        ...
        foreach ($this->_session_data as $key => $value) {
            switch ($key) {
                case 'id' :
                    // session ID
                    $this->_session_data['id'] = $this->_sid;
                    break;
                case 'ip_address' :
                    // visitor ip address
                    $this->_session_data['ip_address'] = $this->_visitor_ip();
                    break;
                case 'user_agent' :
                    // visitor user_agent
                    $this->_session_data['user_agent'] = $this->_user_agent;
                    break;
                case 'init_access' :
                ...
                case 'last_access' :
                    // current access time
                    $this->_session_data['last_access'] = $this->_time;
                    break;
                case 'expiration' :
                ...
                case 'user_id' :
                    ...
                    $this->_session_data['user_id'] = $this->_wp_user_id();
                    break;
                case 'pages_visited' :
    ...
    public function garbage_collection()
    {
        ...
        if ( ! defined('DOING_AJAX') || ! DOING_AJAX) {
            ...
            $expired_session_transient_delete_query_limit = absint(apply_filters('FHEE__EE_Session__garbage_collection___expired_session_transient_delete_query_limit',
                50));
            ...
            if ($expired_session_transient_delete_query_limit) {
				...
				";
                    ...
                    do_action('FHEE__EE_Session__garbage_collection___end',
