...
class EEH_Activation implements ResettableInterface
{
     ...
     * @return stdClass
     */
    public static function migrate_old_config_data($settings = [], $config = '', EE_Config $EE_Config = null)
    {
        $EE_Config          = $EE_Config instanceof EE_Config ? $EE_Config : $EE_Config::instance();
    ...
    public static function get_default_creator_id()
    {
        ...
        $query            = $wpdb->prepare(
            "SELECT user_id FROM $wpdb->usermeta WHERE meta_key = '$capabilities_key' AND meta_value LIKE %s ORDER BY user_id LIMIT 0,1",
            '%' . $role_to_check . '%'
        );
