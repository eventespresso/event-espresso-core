...
class EEH_Parse_Shortcodes
{
     ...
     * @var EE_Shortcodes[]
     */
    private array $shortcode_objs = [];


    /**
     * This kicks off the parsing of shortcodes in message templates
     *
     * @param array|string          $template         This is the incoming string to be parsed
    ...
    public function parse_message_template(
        ...
        $this->template = is_array($template) ? $template['main'] : $template;
    ...
    public function parse_attendee_list_template(
        ...
        $this->template = is_array($template) ? $template['attendee_list'] : $template;
        return $this->_parse_message_template();
    }


    public function parse_event_list_template(
        $template,
        EE_Event $event,
        array $valid_shortcodes,
        array $extra_data = []
        ...
        $this->template = is_array($template) ? $template['event_list'] : $template;
        return $this->_parse_message_template();
    }


    public function parse_ticket_list_template(
        $template,
        EE_Ticket $ticket,
        array $valid_shortcodes,
        array $extra_data = []
        ...
        $this->template = is_array($template) ? $template['ticket_list'] : $template;
        return $this->_parse_message_template();
    }
    ...
    public function parse_line_item_list_template(
        ...
        $this->template = is_array($template) ? $template['ticket_line_item_no_pms'] : $template;
        return $this->_parse_message_template();
    }
    ...
    public function parse_payment_list_template(
        ...
        $this->template = is_array($template) ? $template['payment_list'] : $template;
        return $this->_parse_message_template();
    }
    ...
    public function parse_datetime_list_template(
        ...
        $this->template = is_array($template) ? $template['datetime_list'] : $template;
        return $this->_parse_message_template();
    }


    public function parse_question_list_template(
        $template,
        EE_Answer $answer,
        array $valid_shortcodes,
        array $extra_data = []
        ...
        $this->template = is_array($template) ? $template['question_list'] : $template;
        return $this->_parse_message_template();
    }


    private function _init_data($template, $data, array $valid_shortcodes, $extra_data = [])
    {
        $this->_reset_props();
    ...
    private function _parse_message_template(): string
    {
        ...
        preg_match_all(EE_Shortcodes::REGEX_SHORTCODE_FULL, $this->template, $matches);
        $shortcodes = $matches[0]; // this should be an array of shortcodes in the template string.

        $matched_code = [];
        $sc_values    = [];

        $list_type_shortcodes = (array) apply_filters(
        ...
        // now let's go ahead and loop through our parsers for each shortcode and set up the values
        foreach ($shortcodes as $shortcode) {
            ...
            foreach ($this->shortcode_objs as $sc_obj) {
                ...
                // if this isn't a "list" type shortcode,
                // then we'll send along the data vanilla instead of in an array.
                if (! in_array($shortcode, $list_type_shortcodes)) {
                    $data_send = ! is_object($this->data) && isset($this->data['data'])
                        ? $this->data['data']
                        : $this->data;
                } else {
                    $data_send = $this->data;
                ...
                if ($this->_is_conditional_shortcode($shortcode)) {
                    ...
                    // and will return an empty string, so we need to make sure that
                    // we're only replacing the template when there is a non-empty string.
                    $parsed = $sc_obj->parser($shortcode, $data_send, $this->data['extra_data']);
                    if ($parsed) {
        ...
        // now we've got parsed values for all the shortcodes in the template,
        // so we can go ahead and swap the shortcodes out.
        return str_replace(
            array_values($matched_code),
            array_values($sc_values),
            $this->template
        );
    }
    ...
    private function _set_shortcodes(array $valid_shortcodes)
    {
        foreach ($valid_shortcodes as $shortcode_ref) {
            ...
            if (class_exists($classname) && ! isset($this->shortcode_objs[ $classname ])) {
                $this->shortcode_objs[ $classname ] = new $classname();
            }
        }
    }
}
