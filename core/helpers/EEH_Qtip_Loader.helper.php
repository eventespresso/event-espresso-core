...
class EEH_Qtip_Loader extends EEH_Base {
	...
	private function _register( $config, $paths ) {
		...
		if ( ! $qtip instanceof EE_Qtip_Config )
			throw new EE_Error( sprintf( esc_html__( 'The class given for the Qtip loader (%s) is not a child of the %2$sEE_Qtip_Config%3$s class. Please make sure you are extending EE_Qtip_Config.', 'event_espresso'), $config, '<strong>', '</strong>' ) );
