...
class EEH_Sideloader extends EEH_Base
{
    ...
    public function sideload()
    {
        ...
        if (!$temp_file) {
            EE_Error::add_error(esc_html__('Something went wrong with the upload.  Unable to create a tmp file for the uploaded file on the server', 'event_espresso'), __FILE__, __FUNCTION__, __LINE__);
            return false;
        }
        ...
        if (false === @ rename($file, $path)) {
            unlink($temp_file);
            EE_Error::add_error(sprintf(esc_html__('Unable to move the file to new location (possible permissions errors). This is the path the class attempted to move the file to: %s', 'event_espresso'), $path), __FILE__, __FUNCTION__, __LINE__);
            return false;
        }
