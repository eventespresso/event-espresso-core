...
class EE_Line_Item extends EE_Base_Class implements EEI_Line_Item
{
     ...
     */
    public function children($query_params = [])
    {
        if ($this->ID()) {
     ...
     * @throws ReflectionException
     */
    public function add_child_line_item($line_item, $set_order = true)
    {
    ...
    {
        $grand_total_line_item = EEH_Line_Item::find_transaction_grand_total_for_line_item($this);
        list($total) = $this->calculator->recalculateLineItemTotals($grand_total_line_item);
        return (float) $total;
    }
