...
class EE_Question_Form_Input
{
     ...
     * @return boolean
     */
    private function _question_form_input_property_exists($property)
    {
     ...
     * @throws ReflectionException
     */
    public static function generate_question_form_inputs_for_object($object, $input_types = [])
    {
        $inputs = [];
        ...
        foreach ($fields as $field_ID => $field) {
            if ($field instanceof EE_Model_Field_Base) {
                if (isset($input_types[ $field_ID ])) {
                    // label to display
                    $label = isset($input_types[ $field_ID ]['label']) ? $input_types[ $field_ID ]['label'] : $field->get_nicename();
                    // get saved value for field
                    $value = $object->get($field_ID);
                    // if no saved value, then use default
                    $value = isset($value) ? $value : $field->get_default_value();
                    // determine question type
                    $type = isset($input_types[ $field_ID ]['type']) ? $input_types[ $field_ID ]['type'] : 'TEXT';
                    // input name
                    $input_name = isset($input_types[ $field_ID ]['input_name'])
                        ? $input_types[ $field_ID ]['input_name'] . '[' . $field_ID . ']'
                        : $field_ID;
                    // css class for input
                    $class = isset($input_types[ $field_ID ]['class']) ? $input_types[ $field_ID ]['class'] : '';
                    // css class for label
                    $label_class = isset($input_types[ $field_ID ]['label_class']) ? $input_types[ $field_ID ]['label_class'] : '';
     ...
     * @return void
     */
    public function add_temp_option($QSO)
    {
