	...
	public static function register( $config_class_name, $config_name = '' ) {
		...
		if ( did_action( 'AHEE__EE_System__load_core_configuration__begin' ) ) {
			EE_Error::doing_it_wrong(
				__METHOD__,
				sprintf( __('An attempt made to register "%s" as an EE_Config object has failed because it was not registered at the correct runpoint.  Please regsiter before the "AHEE__EE_System__load_core_configuration__begin" hook has fired', 'event_espresso'), $config_name ),
				'4.3'
				);
		}
	...
	public static function set_config( $addons_config ) {
		foreach ( self::$_ee_config_registry as $class_name => $config_name ) {
			...
			if ( ! class_exists( $class_name ) ) {
				throw EE_Error( sprintf( 'The config class with the name "%s" has cannot be registered with EE_Config because the class does not exist.  Verify that a autoloader has been set for this class', 'event_espresso'), $class_name );
			}
			$name = empty( $config_name ) ? $class_name : $config_name;
			$addons_config->{$name} = new $class_name;
		}
