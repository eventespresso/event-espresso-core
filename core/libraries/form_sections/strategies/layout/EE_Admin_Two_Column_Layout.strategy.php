...
class EE_Admin_Two_Column_Layout extends EE_Two_Column_Layout{
	...
	public function layout_input( $input ) {
		if( $input->get_display_strategy() instanceof EE_Checkbox_Display_Strategy ||
			...
			//leave these alone
		}elseif( $input->get_display_strategy() instanceof EE_Text_Area_Display_Strategy ){
			$input->set_html_class( 'large-text' );
			if( $input instanceof EE_Text_Area_Input ){
				$input->set_rows( 4 );
				$input->set_cols( 60 );
			}
		}else{//standard-like textbox
			$input->set_html_class( 'large-text' );
		}
