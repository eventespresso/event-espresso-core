...
class EE_Max_Length_Validation_Strategy extends EE_Validation_Strategy_Base
{
    ...
    public function __construct($validation_error_message = null, $max_length = EE_INF)
    {
        ...
        if ($validation_error_message === null) {
            $validation_error_message = sprintf(
                esc_html__('Input is too long. Maximum number of characters is %1$s', 'event_espresso'),
                $max_length
            );
        }
    ...
    {
        if (
            $this->_max_length !== EE_INF && $normalized_value && is_string($normalized_value)
            && strlen(
                   $normalized_value
               ) > $this->_max_length
        ) {
    ...
    public function get_jquery_validation_rule_array(): array
    {
        if ($this->_max_length !== EE_INF) {
            return [
                ...
                'messages'  => ['maxlength' => $this->get_validation_error_message()],
            ];
        } else {
            return [];
        }
