...
class EE_Enum_Validation_Strategy extends EE_Validation_Strategy_Base{
	...
	function validate($normalized_value) {
		...
		if( $normalized_value !== NULL && ! array_key_exists($normalized_value,$this->_enum_options)){
			$this->_input->add_validation_error(sprintf(__("'%s' is not allowed option. Allowed options are %s", "event_espresso"),$normalized_value,implode(", ",$this->_enum_options)), 'invalid_enum_value');
			return false;
		}else{
			return true;
		}
