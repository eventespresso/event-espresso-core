<?php namespace EventEspresso\Batch;

use EventEspresso\Batch\Batch_Job_Handlers\BatchJobHandlerInterface;
use EventEspresso\Batch\Helpers\BatchJobHandlerException;
 ...
 * the request to the appropriate BatchJobHandler, which processes a step of the batch,
...
class BatchRunner {
	 ...
	 * @param string $batch_job_handler_class of an auto-loaded class implementing BatchJobHandlerInterface
	 * @param array $request_data to be used by the batch job handler
	 * @return string job_id
	 * @throws BatchJobHandlerException
	 */
	public function create_job( $batch_job_handler_class, $request_data ) {
		...
		if( ! $success ) {
			throw new BatchJobHandlerException(
				sprintf( 
	...
	public function continue_job( $job_id ) {
		...
		if( isset( $response[ 'status' ] ) &&
				$response[ 'status' ] === BatchJobHandlerInterface::status_complete ) {
	 ...
	 * @param type $job_id
	 * @return array {
	 *	@type string $classname name of the autoloaded class implementing BatchJobHandlerInterface
	 *	@type int
	 * @throws BatchJobHandlerException
	 */
	 ...
	 * @return BatchJobHandlerInterface
	 * @throws BatchJobHandlerException
	 */
	public function instantiate_batch_job_handler_from_classname( $classname ) {
		if( ! class_exists( $classname ) ) {
			throw new BatchJobHandlerException(
					sprintf( 
						__('The class %1$s does not exist, and so could not be used for running a job. It should implement BatchJobHandlerInterface.', 'event_espresso'),
						$classname
						) );
			
		}
		$obj = new $classname;
		if( ! $obj instanceof BatchJobHandlerInterface ) {
			throw new BatchJobHandlerException(
				sprintf( 
					__('The class %1$s does not implement BatchJobHandlerInterface and so could not be used for running a job', 'event_espresso'),
					$classname
					) );
	...
	public function finish_job( $job_id ) {
		//get the wordpress option again and the class
		//and calls its finish_job method
	}
}
