...
class PreviewDeletion
{
    ...
    public function handle($request_data, $admin_base_url)
    {
        ...
        $event_ids                = isset($models_and_ids_to_delete['Event']) ? $models_and_ids_to_delete['Event'] : [];
        if (empty($event_ids) || ! is_array($event_ids)) {
            throw new EE_Error(
                esc_html__('No Events were found to delete.', 'event_espresso')
            );
        }
        $datetime_ids = isset($models_and_ids_to_delete['Datetime']) ? $models_and_ids_to_delete['Datetime'] : [];
        if (! is_array($datetime_ids)) {
            throw new UnexpectedEntityException($datetime_ids, 'array');
        }
        $registration_ids = isset($models_and_ids_to_delete['Registration']) ? $models_and_ids_to_delete['Registration'] : [];
