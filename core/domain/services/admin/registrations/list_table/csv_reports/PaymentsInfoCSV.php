...
class PaymentsInfoCSV
{
     ...
     * @return array
     */
    public static function extractPaymentInfo($payments_info)
    {
        $payment_methods = [];
        $gateway_txn_ids_etc = [];
        $payment_times = [];
        foreach ($payments_info as $payment_method_and_gateway_txn_id) {
            $payment_methods[] = isset($payment_method_and_gateway_txn_id['name']) ? $payment_method_and_gateway_txn_id['name'] : esc_html__('Unknown', 'event_espresso');
