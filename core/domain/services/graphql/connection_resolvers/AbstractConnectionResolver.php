...
abstract class AbstractConnectionResolver extends WPGraphQLConnectionResolver
{
     ...
     * @return array
     */
    protected function mapOrderbyInputArgs($query_args, $where_params, $primary_key)
    {
        // ID of the current offset
        $cursor = isset($this->args['after']) ? $this->args['after'] : null;
        $cursor = $cursor ?: (isset($this->args['before']) ? $this->args['before'] : null);
     ...
     * @return array
     */
    protected function sanitizeWhereArgsForInputFields($where_args, $arg_mapping, $id_fields = [])
    {
     ...
     * @return string
     */
    protected function getSearchKeywords($where_args)
    {
        $search = '';
