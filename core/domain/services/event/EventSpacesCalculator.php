...
class EventSpacesCalculator
{
     ...
     * @throws ReflectionException
     */
    public function setActiveTickets($active_tickets = [])
    {
    ...
    {
        $consider_sold = (bool) $consider_sold;
        if ($this->debug) {
            EEH_Debug_Tools::printr(__FUNCTION__, __CLASS__, __FILE__, __LINE__, 2);
    ...
    {
        $datetime_identifier = (string) $datetime_identifier;
        // make sure a reg limit is set for the datetime
        $reg_limit = isset($this->datetime_spaces[ $datetime_identifier ]) ? $this->datetime_spaces[ $datetime_identifier ] : 0;
    ...
    private function calculateAvailableSpacesForTicket(
        ...
        $datetime_identifier = (string) $datetime_identifier;
        $ticket_identifier = (string) $ticket_identifier;
        // make sure ticket quantity is set
        $ticket_quantity = isset($this->ticket_quantities[ $ticket_identifier ]) ? $this->ticket_quantities[ $ticket_identifier ] : 0;
        if ($this->debug) {
