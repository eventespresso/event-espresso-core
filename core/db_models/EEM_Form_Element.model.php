...
class EEM_Form_Element extends EEM_Base
{
     ...
     * @throws EE_Error
     */
    protected function __construct(FormStatus $form_status, InputTypes $input_types, $timezone)
    {
     ...
     */
    public function filterFormElementsForFormSection($form_section, $all_form_elements)
    {
        return array_filter($all_form_elements, ($form_section->formElementFilter() === null ? function ($v, $k) {
            return !empty($v);
        } : $form_section->formElementFilter()) === null ? function ($v, $k) {
            return !empty($v);
        } : ($form_section->formElementFilter() === null ? function ($v, $k) {
            return !empty($v);
        } : $form_section->formElementFilter()), ($form_section->formElementFilter() === null ? function ($v, $k) {
            return !empty($v);
        } : $form_section->formElementFilter()) === null ? ARRAY_FILTER_USE_BOTH : ($form_section->formElementFilter() === null ? ARRAY_FILTER_USE_BOTH : 0));
    }
     ...
     * @throws ReflectionException
     */
    public function getAllFormElementsForFormSections($form_sections)
    {
        $FSC_UUIDs = [];
