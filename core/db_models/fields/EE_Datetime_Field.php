...
class EE_Datetime_Field extends EE_Model_Field_Base
{
     ...
     * @access public
     *
     * @param string $what acceptable values are 'time' or 'date'.  Any other value will be set but will always result
    ...
    protected function _prepare_for_display($DateTime, $schema = false)
    {
        if ( ! $DateTime instanceof DateTime) {
            if ($this->_nullable) {
                ...
                if (WP_DEBUG) {
                    throw new EE_Error(sprintf(__('EE_Datetime_Field::_prepare_for_display requires a DateTime class to be the value for the $DateTime argument because the %s field is not nullable.',
                        'event_espresso'), $this->_nicename));
                } else {
                    $DateTime = new DbSafeDateTime(\EE_Datetime_Field::now);
                    EE_Error::add_error(sprintf(__('EE_Datetime_Field::_prepare_for_display requires a DateTime class to be the value for the $DateTime argument because the %s field is not nullable.  When WP_DEBUG is false, the value is set to "now" instead of throwing an exception.',
                        'event_espresso'), $this->_nicename));
                }
            }
        }
    ...
    public function prepare_for_use_in_db($datetime_value)
    {
        ...
        if ( ! empty($datetime_value) && ! $datetime_value instanceof DateTime) {
            throw new EE_Error(__('The incoming value being prepared for setting in the database must either be empty or a php DateTime object',
                'event_espresso'));
        }
     ...
     *
     * To determine this we check if the set timezone offset is different than the blog's set timezone offset.  If so,
     * then true.
     *
