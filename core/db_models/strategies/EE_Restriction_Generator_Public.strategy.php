	...
	protected function _generate_restrictions() {
		...
		if( self::is_cap($this->model(), $this->action()) ) {
			if( $this->model() instanceof EEM_CPT_Base ){
				$restrictions[ self::get_cap_name($this->model(), $this->action()) ] = new EE_Default_Where_Conditions( array( 'status' => 'publish' ) );
			}elseif( $this->model() instanceof EEM_Soft_Delete_Base ) {
				$restrictions[ self::get_cap_name($this->model(), $this->action()) ] = new EE_Default_Where_Conditions( array( $this->model()->deleted_field_name() => false ) );
			}else{
			...
			if( self::is_cap($this->model(), $this->action() . '_others' ) ) {//both caps exist
				if( $this->model() instanceof EEM_CPT_Base ) {
					...
					$restrictions[ self::get_cap_name($this->model(), $this->action() . '_others' ) ] = new EE_Default_Where_Conditions(
							array(
								'OR*' . self::get_cap_name($this->model(), $this->action() . '_others' ) => array(
				...
				if( self::is_cap( $this->model(), $this->action() . '_private' ) && $this->model() instanceof EEM_CPT_Base ){
					...
					$restrictions[ self::get_cap_name($this->model(), $this->action() . '_private' ) ] = new EE_Default_Where_Conditions(
							array(
								'OR*' . self::get_cap_name($this->model(), $this->action() . '_private' ) => array(
									EE_Default_Where_Conditions::user_field_name_placeholder => EE_Default_Where_Conditions::current_user_placeholder,
