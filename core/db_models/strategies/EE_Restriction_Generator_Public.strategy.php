		...
		if( self::is_cap($model, $action) ) {
			...
			if( self::is_cap($model, $action . '_others' ) ) {//both caps exist
				if( $model instanceof EEM_CPT_Base ) {
					...
					$restrictions[ self::get_cap_name($model, $action . '_others' ) ] = new EE_Owner_And_Others_Where_Conditions( array( 'status' => 'publish' ) );
				}elseif( $model instanceof EEM_Soft_Delete_Base ){
					//then if they don't have the othehr cap, AT MOST show them their own or undeleted ones
					$restrictions[ self::get_cap_name($model, $action . '_others' ) ] = new EE_Owner_And_Others_Where_Conditions( array( 'deleted' => false ) );
				}else{
				...
				if( self::is_cap( $model, $action . '_private' ) && $model instanceof EEM_CPT_Base ){
					...
					$restrictions[ self::get_cap_name($model, $action . '_private' ) ] = new EE_Owner_And_Others_Where_Conditions( array( 'status' => array( '!=', 'private' ) ) );
					;
				}
