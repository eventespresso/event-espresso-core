...
class EE_Restriction_Generator_Event_Related_Public extends EE_Restriction_Generator_Base {
	...
	protected function _generate_restrictions() {
		...
		return array(
			...
			self::get_cap_name($event_model, $this->action()) => new EE_Default_Where_Conditions( array( $this->_path_to_event_model . 'status' => 'publish' ) ),
			//if they don't have the others event cap, they can't access others' non-default items
			self::get_cap_name($event_model, $this->action() . '_others' ) => new EE_Default_Where_Conditions(
							array(
								'OR*' . self::get_cap_name($event_model, $this->action() . '_others' ) => array(
