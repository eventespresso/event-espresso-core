...
/**
 * The current model has the foreign key pointing to the other model, but the foreign key can point to ANY model object (roughly any table)
 * specified on the field. Note: Also requires this model to have a field of type EE_Any_Foreign_Model_Name_Field,
...
class EE_Belongs_To_Any_Relation extends EE_Belongs_To_Relation {
	...
	function add_relation_to($this_obj_or_id, $other_obj_or_id) {
		...
		$this_model_obj->set($this->get_this_model()->get_field_containing_related_model_name()->get_name(), $this->get_other_model()->get_this_model_name());
		$this_model_obj->save();
