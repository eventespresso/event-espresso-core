...
class EEM_Price extends EEM_Soft_Delete_Base
{
    ...
    protected function __construct(?string $timezone = '')
    {
        ...
        $this->_fields       = [
            'Price' => [
                ...
                'PRC_order'      => new EE_Integer_Field(
                    'PRC_order',
                    esc_html__('Order of Application of Price (lower numbers apply first?)', 'event_espresso'),
                    false,
                    1
                ),
        ...
        $caps                        = [];
        $caps[ EEM_Base::caps_read ] = new EE_Restriction_Generator_Default_Public(
            'PRC_is_default',
            'Ticket.Datetime.Event'
        );
        // account for default tickets in the caps
        $caps[ EEM_Base::caps_read_admin ] = new EE_Restriction_Generator_Default_Protected(
            'PRC_is_default',
            'Ticket.Datetime.Event'
        );
        $caps[ EEM_Base::caps_edit ]       = new EE_Restriction_Generator_Default_Protected(
            'PRC_is_default',
            'Ticket.Datetime.Event'
        );
        $caps[ EEM_Base::caps_delete ]     = new EE_Restriction_Generator_Default_Protected(
            'PRC_is_default',
            'Ticket.Datetime.Event'
        );
    ...
    {
        // if there is no tkt
        $ticket_prices = empty($TKT_ID)
            // get prices with no tkt ID, are global, are not a tax, and are active
            ? $this->get_all_default_prices()
            : $this->get_all(
                [
                    [
                        'TKT_ID'      => $TKT_ID,
