...
class LineItemCalculator
{
     ...
     */
    public function recalculateTotalIncludingTaxes($line_item, $update_txn_status = false)
    {
        ...
        list(, $pretax_total) = $this->recalculateLineItemTotals($line_item);
    ...
    public function recalculateLineItemTotals(
        ...
        switch ($line_item->type()) {
            ...
            case EEM_Line_Item::type_sub_total:
                list($total, $pretax_total) = $this->recalculateSubTotal($line_item);
                break;

            case EEM_Line_Item::type_line_item:
                list($total, $pretax_total) = $this->recalculateLineItem($line_item, $total, $pretax_total);
                break;

            case EEM_Line_Item::type_sub_line_item:
                // sub line items operate on the total and update both the total AND the pre-tax total
                list($total, $pretax_total) = $this->recalculateSubLineItem($line_item, $total, $pretax_total);
                break;

            case EEM_Line_Item::type_sub_tax:
                // sub line item taxes ONLY operate on the pre-tax total and ONLY update the total
                list($total, $pretax_total) = $this->recalculateSubTax($line_item, $pretax_total);
                break;
     ...
     * @throws ReflectionException
     */
    private function recalculateSubTotal(EE_Line_Item $line_item)
    {
        ...
        foreach ($children as $child_line_item) {
            list($child_total, $child_pretax_total) = $this->recalculateLineItemTotals(
                $child_line_item,
                $total,
                $pretax_total
            );
    ...
    private function recalculateLineItem(
        ...
        $total = (double) $total;
        $pretax_total = (double) $pretax_total;
     ...
     * @throws ReflectionException
     */
    private function recalculateSubLineItem(EE_Line_Item $sub_line_item, $total = 0, $pretax_total = 0)
    {
        $total = (double) $total;
        $pretax_total = (double) $pretax_total;
     ...
     * @throws ReflectionException
     */
    private function recalculateSubTax(EE_Line_Item $sub_line_item, $pretax_total = 0)
    {
        $pretax_total = (double) $pretax_total;
     ...
     * @throws ReflectionException
     */
    private function updateLineItemQuantities(EE_Line_Item $line_item, $quantity = 1)
    {
     ...
     * @return float
     */
    private function calculatePercentage($total, $percent, $round = false)
    {
        $total = (double) $total;
     ...
     * @throws ReflectionException
     */
    private function calculateTotalForQuantity(EE_Line_Item $line_item)
    {
     ...
     * @throws ReflectionException
     */
    private function updatePercent(EE_Line_Item $line_item, $percent)
    {
        $percent = (double) $percent;
     ...
     * @throws ReflectionException
     */
    private function updatePreTaxTotal(EE_Line_Item $line_item, $pretax_total, $round = false)
    {
        $pretax_total = (double) $pretax_total;
        $round = (bool) $round;
     ...
     * @throws ReflectionException
     */
    private function updateQuantity(EE_Line_Item $line_item, $quantity)
    {
        $quantity = (int) $quantity;
     ...
     * @throws ReflectionException
     */
    private function updateTotal(EE_Line_Item $line_item, $total, $round = false)
    {
        $total = (double) $total;
        $round = (bool) $round;
     ...
     * @throws ReflectionException
     */
    private function updateTransaction(EE_Line_Item $line_item, $total, $update_status)
    {
        $total = (double) $total;
     ...
     * @throws ReflectionException
     */
    private function updateUnitPrice(EE_Line_Item $line_item, $pretax_total)
    {
        $pretax_total = (double) $pretax_total;
        $quantity = $line_item->quantity();
     ...
     * @throws ReflectionException
     */
    public function recalculateTaxesAndTaxTotal($total_line_item)
    {
     ...
     * @throws ReflectionException
     */
    private function applyGlobalTaxes(EE_Line_Item $total_line_item, $taxable_total)
    {
        $taxable_total = (double) $taxable_total;
        $this->validateLineItemAndType($total_line_item, EEM_Line_Item::type_total);
        $total_tax = 0;
    ...
    private function calculateNonGlobalTaxes(
        ...
        $line_item_total = (double) $line_item_total;
     ...
     * @throws ReflectionException
     */
    public function getTotalTax($line_item)
    {
        $this->validateLineItemAndType($line_item, EEM_Line_Item::type_total);
     ...
     * @throws ReflectionException
     */
    public function taxableAmountForGlobalTaxes($line_item)
    {
        $total      = 0;
     ...
     * @throws ReflectionException
     */
    private function validateLineItemAndType($line_item, $type = null)
    {
