...
abstract class EE_Admin_Page extends EE_Base implements InterminableInterface
{
    ...
    protected function _verify_routes()
    {
        ...
        if (array_key_exists($this->_req_action, $this->_page_routes)) {
            ...
            $this->_route_config = isset($this->_page_config[ $this->_req_action ]) ? $this->_page_config[ $this->_req_action ] : [];
        } else {
            // user error msg
            $error_msg = sprintf(
                esc_html__(
    ...
    protected function _route_admin_request()
    {
        ...
        if (! empty($func)) {
            ...
            list($class, $method) = $route_callback;
    ...
    protected function _set_nav_tabs()
    {
        ...
        foreach ($this->_page_config as $slug => $config) {
            ...
            $this->_nav_tabs[ $slug ] = [
                'url'       => isset($config['nav']['url']) ? $config['nav']['url'] : EE_Admin_Page::add_query_args_and_nonce(
                        ['action' => $slug],
                        $this->_admin_base_url
                    ),
     ...
     * @return string
     */
    private function navTabLabel(array $nav_tab, $slug)
    {
        $slug = (string) $slug;
        $label = isset($nav_tab['label']) ? $nav_tab['label'] : ucwords(str_replace('_', ' ', $slug));
        $icon  = isset($nav_tab['icon']) ? $nav_tab['icon'] : null;
     ...
     * @since   $VID:$
     */
    protected function getPublishBoxTitle()
    {
        $publish_box_title = esc_html__('Publish', 'event_espresso');
        if (! empty($this->_labels['publishbox'])) {
            if (is_array($this->_labels['publishbox'])) {
                $publish_box_title = isset($this->_labels['publishbox'][ $this->_req_action ]) ? $this->_labels['publishbox'][ $this->_req_action ] : $publish_box_title;
            } else {
    ...
    protected function _set_publish_post_box_vars(
        ...
        // add hidden fields
        $this->_template_args['publish_hidden_fields'] = isset($this->_template_args['publish_hidden_fields']) ? $this->_template_args['publish_hidden_fields'] : '';
        foreach ($hidden_fields as $hidden_field) {
            $this->_template_args['publish_hidden_fields'] .= isset($hidden_field['field']) ? $hidden_field['field'] : '';
        }
    }
     ...
     */
    protected function addPublishPostMetaBoxHiddenFields($field_name, $field_attributes)
    {
    ...
    private function _display_admin_page($sidebar = false, $about = false)
    {
        ...
        $post_body_content = isset($this->_template_args['before_admin_page_content']) ? $this->_template_args['before_admin_page_content'] : '';
     ...
     */
    public function set_hook_object($hook_obj)
    {
     ...
     * @return array
     */
    protected function mergeExistingRequestParamsWithRedirectArgs($new_route_data)
    {
     ...
     * @return bool success/fail
     */
    protected function _process_payment_notification($payment)
    {
