<?php
/**
 * Contains migrations for EE 4.4 model
 ...
 */
$stages = glob(EE_CORE.'data_migration_scripts/4_4_0_stages/*');
$class_to_filepath = array();
foreach($stages as $filepath){
	$matches = array();
	preg_match('~4_4_0_stages/(.*).dmsstage.php~',$filepath,$matches);
	$class_to_filepath[$matches[1]] = $filepath;
}
//give addons a chance to autoload their stages too
$class_to_filepath = apply_filters('FHEE__EE_DMS_4_4_0__autoloaded_stages',$class_to_filepath);
EEH_Autoloader::register_autoloader($class_to_filepath);


/**
 * EE_DMS_Core_4_4_0
 * meant to convert DBs between 4.3.x to 4.4.0
 * The following is done in this migration script:
 * -Create esp_event_message_template table
 * -Migrate any existing custom message templates attached to events to the new table;
 *
 * @package 		Event Espresso
 * @subpackage 	migrations
 * @since 			4.4
 ...
 */
class EE_DMS_Core_4_4_0 extends EE_Data_Migration_Script_Base{



	public function __construct() {
		$this->_pretty_name = __("Data Migration to Event Espresso 4.4.0.P", "event_espresso");
		$this->_migration_stages = array(
			new EE_DMS_4_4_0_event_message_templates()
		);
		parent::__construct();
	}
	public function can_migrate_from_version($version_array) {
		$version_string = $version_array['Core'];
		if($version_string <= '4.4.000' && $version_string >= '4.3.0' ){
//			echo "$version_string can be mgirated from";
			return true;
		}elseif( ! $version_string ){
//			echo "no version string provided: $version_string";
			//no version string provided... this must be pre 4.3
			//because since 4.3 we're
