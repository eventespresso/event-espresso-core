...
class EE_DMS_Core_4_9_0 extends EE_Data_Migration_Script_Base
{
    ...
    public function can_migrate_from_version($version_array)
    {
        ...
        if (
            version_compare($version_string, '4.9.0.decaf', '<') &&
    ...
    public function verify_db_collations()
    {
        ...
        if (
            'utf8mb4' !== $wpdb->charset || get_option('ee_verified_db_collations', false)
        ) {
            return;
        }
                ...
                if ($model_obj instanceof EEM_Base) {
                    foreach ($model_obj->get_tables() as $table) {
                        if (
                            ...
                            (is_main_site()
                             // for main tables, verify global tables
                             ||
                             ! $table->is_global(
                             )// if not the main site, then only verify non-global tables (avoid doubling up)
                            ) &&
    ...
    /**
     * Verifies DB collations because a bug was discovered on
     * https://events.codebasehq.com/projects/event-espresso/tickets/10435 which meant some DB collations might not
     * have been updated
     *
     * @return void
     */
    public function verify_db_collations_again()
    {
        ...
        if (
            'utf8mb4' !== $wpdb->charset || get_option('ee_verified_db_collations_again', false)
        ) {
            return;
        }
        $tables_to_check = [
    ...
    protected function _verify_db_collations_for_tables($tables_to_check)
    {
        foreach ($tables_to_check as $table_name) {
            ...
            if (
                ! apply_filters(
                    'FHEE__EE_DMS_Core_4_9_0__verify_db_collations__check_overridden',
                    false,
                    $table_name
