 ...
 * 'LIN_order'=>new EE_Integer_Field('LIN_order', __("Order of Application towards total of parent", "event_espresso"),
 * false,1),
 ...
 * 'LIN_parent'=>new EE_Integer_Field('LIN_parent', __("Parent ID (this item goes towards that Line Item's total)",
 * "event_espresso"), true, null),
 ...
 * 'OBJ_ID'=>new EE_Foreign_Key_Int_Field('OBJ_ID', __("ID of Item purchased.", "event_espresso"),
 * true,null,$line_items_can_be_for),
 * 'OBJ_type'=>new EE_Any_Foreign_Model_Name_Field('OBJ_type', __("Model Name this Line Item is for",
...
class EE_DMS_4_1_0_line_items extends EE_Data_Migration_Script_Stage_Table
{
    ...
    protected function _migrate_old_row($old_row)
    {
        ...
        if (intval($old_row['is_primary'])) {
            ...
            if (! $txn_id) {
                $this->add_error(
                    sprintf(
                        __("Could not find the transaction for the 3.1 attendee %d from row %s", "event_espresso"),
                        $old_row['id'],
    ...
    private function _get_txn($txn_id)
    {
        ...
        $txn =
            $wpdb->get_row(
                $wpdb->prepare("SELECT * FROM $this->_new_transaction_table WHERE TXN_ID=%d", $txn_id),
                ARRAY_A
            );
        return $txn;
    }
    ...
    {
        global $wpdb;
        $regs_on_this_transaction =
            $wpdb->get_results(
                $wpdb->prepare("SELECT * FROM " . $this->_new_reg_table . " WHERE TXN_ID=%d", $transaction['TXN_ID']),
                ARRAY_A
            );
        $new_line_item_ids        = [];
    ...
    private function _get_new_ticket_row($new_ticket_id)
    {
        ...
        $ticket_row =
            $wpdb->get_row(
                $wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "esp_ticket WHERE TKT_ID=%d", $new_ticket_id),
                ARRAY_A
            );
        return $ticket_row;
    }
