...
class EE_DMS_4_3_0_question_option_order extends EE_Data_Migration_Script_Stage_Table {
	...
	function __construct() {
		...
		$this->_qso_table = $wpdb->prefix."esp_question_option";
		parent::__construct();
	}
	protected function _migrate_old_row($old_row) {
		...
		global $wpdb;
		$updated = $wpdb->update($this->_qso_table,
		...
		if ( FALSE === $updated ) {
			$this->add_error(sprintf(__("Error in updating table {$this->_qso_table} setting QSO_order = %d where QSO_ID = %d", 'event_espresso'),$old_row['QSO_ID'],$old_row['QSO_ID']));
		}
	}
}