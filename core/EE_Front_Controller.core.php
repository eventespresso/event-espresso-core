					...
					foreach ( $post_shortcodes as $shortcode_class => $post_id ) {
						...
						if ( ! isset( EE_Registry::instance()->shortcodes[ $shortcode_class ] )) {
							if ( defined( 'WP_DEBUG' ) && WP_DEBUG === TRUE ) {
								$msg = sprintf( __( 'The [%s] shortcode has not been properly registered or the corresponding addon/module is not active for some reason. Either fix/remove the shortcode from the post, or activate the addon/module the shortcode is associated with. The list of post shortcodes is %s, and the list of registered shortcodes is %s.', 'event_espresso' ), $shortcode_class, '<pre>' . print_r( EE_Registry::instance()->CFG->core->post_shortcodes, TRUE ) . '/<pre>', '<pre>' . print_r( EE_Registry::instance()->shortcodes, TRUE ) . '/<pre>' );
