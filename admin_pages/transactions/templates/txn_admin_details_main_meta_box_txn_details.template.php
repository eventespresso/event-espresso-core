<div id="admin-primary-mbox-dv" class="admin-primary-mbox-dv">
	...
	<a id="display-additional-transaction-session-info" class="display-the-hidden smaller-text" rel="additional-transaction-session-info">
		<span class="dashicons dashicons-plus-alt"></span><?php esc_html_e( 'view additional transaction session details', 'event_espresso' );?>
	</a>
	...
	<div id="additional-transaction-session-info-dv" class="hidden">
		...
		<a id="hide-additional-transaction-session-info" class="hide-the-displayed hidden smaller-text" rel="additional-transaction-session-info">
			<span class="dashicons dashicons-dismiss"></span><?php esc_html_e( 'hide additional transaction session details', 'event_espresso' );?>
		</a>
	<br class="clear"/>

		<h3 class="admin-primary-mbox-h4"><?php esc_html_e( 'Transaction Session Details', 'event_espresso' );?></h3>
	...
	<div class="admin-primary-mbox-tbl-wrap">
		<table id="txn-admin-payments-tbl" class="admin-primary-mbox-tbl">
			<thead>
				<tr>
					...
					<th class="jst-left"><?php esc_html_e( 'Notes / Extra&nbsp;Accounting', 'event_espresso' );?></th>
		...
		<?php if ( $payments ) : ?>
				...
				<tr id="txn-admin-payment-tr-<?php echo $PAY_ID;?>">
					...
					<td class=" jst-left">
						<div id="payment-gateway-<?php echo $PAY_ID;?>">
							<?php echo $payment->payment_method() ?  $payment->payment_method()->admin_name() : esc_html__("Unknown", 'event_espresso');?>
						</div>
			...
			<?php
				...
				$overpaid = $payment_total > $grand_raw_total ? '<span id="overpaid">' . esc_html__( 'This transaction has been overpaid ! ', 'event_espresso' ) . '</span>' : '';
			?>
				...
				<tr id="txn-admin-payments-total-tr" class="admin-primary-mbox-total-tr<?php echo $pay_totals_class;?>">
					<th class=" jst-rght" colspan="10"><span id="payments-total-spn"><?php echo $overpaid . sprintf( esc_html__( 'Payments Total %s', 'event_espresso' ), '(' . EE_Registry::instance()->CFG->currency->code . ')' );?></span></th>
		...
		<?php else : ?>
				...
				<tr id="txn-admin-payments-total-tr" class="admin-primary-mbox-total-tr hidden">
					<th class=" jst-rght" colspan="10"><span id="payments-total-spn"><?php echo esc_html__( 'Payments Total', 'event_espresso' );?></span></th>
	...
	<div id="txn-admin-apply-payment-dv" class="txn-admin-payment-option auto-hide" style="display: none;">
		...
		<form name="txn-admin-apply-payment-frm" id="txn-admin-apply-payment-frm" action="<?php echo $apply_payment_form_url; ?>">
			<div class="admin-modal-dialog-wrap">
				<div class="admin-modal-dialog-inner">
					...
					<div class="txn-admin-apply-payment-date-dv admin-modal-dialog-row">
						<div class="validation-notice-dv"><?php esc_html_e( 'The following is  a required field', 'event_espresso' );?></div>
						<label for="txn-admin-payment-date-inp" class=""><?php esc_html_e( 'Payment Date', 'event_espresso' );?></label>
						...
						<p class="description"><?php esc_html_e( 'The date the payment was actually made on', 'event_espresso' );?></p>
					</div>

					<div class="txn-admin-apply-payment-amount-dv admin-modal-dialog-row">
						<div class="validation-notice-dv"><?php esc_html_e( 'The following is  a required field', 'event_espresso' );?></div>
						<label for="txn-admin-payment-amount-inp" class=""><?php esc_html_e( 'Amount', 'event_espresso' );?></label>
						...
						<p class="description"><?php esc_html_e( 'The amount of the payment', 'event_espresso' );?></p>
					</div>

					<div class="txn-admin-apply-payment-method-dv admin-modal-dialog-row">
						<div class="validation-notice-dv"><?php esc_html_e( 'The following is  a required field', 'event_espresso' );?></div>
						<label for="txn-admin-payment-method-inp" class=""><?php esc_html_e( 'Method of Payment', 'event_espresso' );?></label>
						...
						<p class="description"><?php esc_html_e( 'Whether the payment was made via PayPal, Credit Card, Cheque, or Cash', 'event_espresso' );?></p>
					</div>

					<div class="mop-PP mop-CC mop-CHQ mop">
						<div class="txn-admin-apply-payment-gw-txn-id-dv admin-modal-dialog-row">
							<label for="txn-admin-payment-txn-id-inp" class=""><?php esc_html_e( 'TXN ID / CHQ #', 'event_espresso' );?></label>
							...
							<p class="description"><?php esc_html_e( 'The Transaction ID sent back from the payment gateway, or the Cheque #', 'event_espresso' );?></p>
						</div>
					</div>

					<div class="mop-CC mop" style="display:none">
						<div class="txn-admin-apply-payment-response-dv admin-modal-dialog-row">
							<label for="txn-admin-payment-gateway-response-inp" class=""><?php esc_html_e( 'Gateway Response', 'event_espresso' );?></label>
							...
							<p class="description"><?php esc_html_e( 'The gateway response string (optional)', 'event_espresso' );?></p>
						</div>
					</div>

					<div class="mop-PP mop-CC mop">
						<div class="txn-admin-apply-payment-status-dv admin-modal-dialog-row">
							<label for="txn-admin-payment-status-inp" class=""><?php esc_html_e( 'Payment Status', 'event_espresso' );?></label>
							...
							<p class="description"><?php esc_html_e( 'Whether the payment was approved, cancelled, declined or failed after submission to the gateway', 'event_espresso' );?></p>
						</div>
					</div>

					<div class="txn-admin-apply-payment-po-nmbr-dv admin-modal-dialog-row">
						<label for="txn-admin-payment-po-nmbr-inp" class=""><?php esc_html_e( 'P.O. / S.O. #', 'event_espresso' );?></label>
						...
						<p class="description"><?php esc_html_e( 'The Purchase or Sales Order Number if any (optional)', 'event_espresso' );?></p>
					</div>

					<div class="txn-admin-apply-payment-accounting-dv admin-modal-dialog-row">
						<label for="txn-admin-payment-accounting-inp" class="last"><?php esc_html_e( 'Notes / Extra Accounting', 'event_espresso' );?></label>
						...
						<p class="description"><?php esc_html_e( 'An extra field you may use for accounting purposes or simple notes. Defaults to the primary registrant\'s registration code.', 'event_espresso' );?></p>
					</div>

					<div class="txn-admin-apply-payment-registrations-dv admin-modal-dialog-row">
						<label for="txn-admin-payment-registrations-inp" class="last"><?php esc_html_e( 'Registrations to Apply Payment to:', 'event_espresso' ); ?></label>
					...
					<div class="txn-admin-payment-reg-status-dv admin-modal-dialog-row">
						<label for="txn-admin-payment-reg-status-inp" class="last"><?php esc_html_e( 'Change Registration Status?', 'event_espresso' );?></label>
						<?php echo $status_change_select; ?>
						<p class="description"><?php esc_html_e( 'If you wish to change the status for the registrations selected above, then select which status from this dropdown.', 'event_espresso' ); ?></p>
						<br/>
					</div>

					<div class="txn-admin-apply-payment-send-notifications-dv admin-modal-dialog-row">

						<label for="txn-admin-payment-send-notifications-inp" class="last"><?php esc_html_e( 'Send Related Messages?', 'event_espresso' );?></label>
			...
			<ul id="admin-modal-dialog-options-ul">
				...
				<li>
					<span id="ee-ajax-processing-text" style="display:none;"><?php esc_html_e('Processing...', 'event_espresso'); ?></span>
				</li>
			</ul>
	...
	<div id="txn-admin-delete-payment-dv" class="txn-admin-payment-option auto-hide" style="display: none;">
		...
		<h2 id="admin-modal-dialog-delete-payment-h2" class="admin-modal-dialog-h2 hdr-has-icon" style="display:none;">
			...
			<?php echo esc_html__( 'Delete Payment/Refund for Transaction #', 'event_espresso' ) . $txn_nmbr['value'];?>
		</h2>
		...
		<form name="txn-admin-delete-payment-frm" id="txn-admin-delete-payment-frm" action="<?php echo $delete_payment_url; ?>">
			<div class="admin-modal-dialog-wrap">
				<div class="admin-modal-dialog-inner">
					...
					<div class="txn-admin-apply-payment-accounting-dv admin-modal-dialog-row">
						<label for="delete-txn-admin-payment-reg-status-inp" class="last"><?php esc_html_e( 'Change Registration Status?', 'event_espresso' );?></label>
					...
					<div class="ee-attention txn-admin-apply-payment-accounting-dv admin-modal-dialog-row">
						<label for="delete-txn-admin-payment-accounting-inp" class="last"><?php esc_html_e( 'Send Related Messages?', 'event_espresso' );?></label>
						<input type="checkbox" value="1" name="delete_txn_reg_status_change[send_notifications]">
						<p class="description"><?php esc_html_e( 'If you check this box, the system will send any related registration messages matching the status of the registrations to each registration for this transaction. No Payment notifications are sent when deleting a payment.', 'event_espresso' );?></p>
					</div>
					<div class="clear"></div>
			...
			<ul id="del-admin-modal-dialog-options-ul">
				...
				<li>
					<span id="delete-ee-ajax-processing-text" style="display:none;"><?php esc_html_e('Processing...', 'event_espresso'); ?></span>
				</li>
			</ul>
	...
	if ( WP_DEBUG ) {
		...
		if ( isset( $delivered_messages[ $TXN_ID ] )) {
			?>
			<h4 class="admin-primary-mbox-h4 hdr-has-icon"><span class="dashicons dashicons-email-alt"></span><?php esc_html_e( 'Messages Sent to Primary Registrant', 'event_espresso' );?></h4>
				...
				<table class="admin-primary-mbox-tbl">
					<thead>
						<tr>
							...
							<th class="jst-left"><?php esc_html_e( 'Payment Status Upon Sending', 'event_espresso' );?></th>
