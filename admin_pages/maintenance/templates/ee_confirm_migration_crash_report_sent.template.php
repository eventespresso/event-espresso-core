...
<div class='ee-table-wrap'>
	<table>
		...
		<tbody>
			<tr><td><?php _e("I want to retry migrating my data", 'event_espresso');?></td><td><p><?php printf( __('First, %1$s check the forums %2$s to see if there is a solution before re-attempting to migrate.', 'event_espresso'), "<a href='".EE_SUPPORT_EMAIL."' target='_blank'>", '</a>' );?></p><p><?php printf( __('To retry migrating your data: restore to the backup you made before the migration and reactivate EE (and any addons you are using) and re-run the migration scripts. If you did not make a database migration and are migrating from EE3: delete your EE4 data (use the %1$s Reset/Delete Data tab above%2$s), and then reactivate EE4, and then re-run the migration scripts.', 'event_espresso'), "<a href='$reset_db_page_url'>", "</a>" );?></p></td></tr>
			<tr><td><?php _e("I want to hear from Support before proceeding", 'event_espresso');?></td><td><?php printf( __('Just make sure you\'ve %1$s checked for a solution in the forums,%2$s and properly contacted Support. We will get back to you as soon as posisble', 'event_espresso'), "<a href='".EE_SUPPORT_EMAIL."'>", "</a>" );?></td></tr>
			<tr><td><?php printf( __('I don\'t need my old EE %s data', 'event_espresso'), $most_recent_migration->slug() );?></td><td><?php printf( __('If you are ok with losing all the EE %1$s data, you can skip the migrations and %2$s use EE4 with only default Data%3$s', 'event_espresso'), $most_recent_migration->slug(), "<a id='do-not-migrate' class='do-not-migrate button-primary' href='$reset_db_action_url'>","</a>");?></td></tr>
			<tr><td><p><?php printf( __('I want to go back to my old version of EE %1$s', 'event_espresso'), $most_recent_migration->slug() ) ;?></td><td><?php printf( __('Then we suggest you re-activate the old version of EE %3$s and restore your database to the backup you made just before migrating . If you didn\'t backup your database and are migrating from EE3, you can also delete your EE4 data (use on the %1$s"Reset/Delete Data" tab above%2$s), and then reactivate EE3 from the plugins page. Note: some of your EE3 shortcodes may have been changed to their EE4 equivalents, so you will need to change them back.', 'event_espresso'), "<a href='$reset_db_page_url'>", "</a>", $most_recent_migration->slug() );?></p>
