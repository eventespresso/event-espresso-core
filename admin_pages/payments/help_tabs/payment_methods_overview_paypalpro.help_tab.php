<h3><?php _e('PayPal Pro', 'event_espresso'); ?></h3>
<p>
<?php _e('Adjust the settings for the PayPal Pro payment gateway.', 'event_espresso'); ?>
</p>
<h3><?php _e('PayPal Pro Settings', 'event_espresso'); ?></h3>
<ul>
<li>
<?php _e('PayPal Pro Email', 'event_espresso'); ?><br />
<?php _e('Enter the email that you use to login to your PayPal account.', 'event_espresso'); ?>
</li>
<li>
<?php _e('PayPal API Username', 'event_espresso'); ?><br />
<?php _e('Enter your API Username for PayPal. Learn how to find your <a href="https://www.paypal.com/us/cgi-bin/webscr?cmd=xpt/cps/merchant/wppro/WPProIntegrationSteps-outside#SectionB">API Username</a>.', 'event_espresso'); ?>
</li>
<li>
<?php _e('PayPal API Password', 'event_espresso'); ?><br />
<?php _e('Enter your API Password for PayPal. Learn how to find your <a href="https://www.paypal.com/us/cgi-bin/webscr?cmd=xpt/cps/merchant/wppro/WPProIntegrationSteps-outside#SectionB">API Password</a>.', 'event_espresso'); ?>
</li>
<li>
<?php _e('PayPal API Signature', 'event_espresso'); ?><br />
<?php _e('Enter your API Signature for PayPal. Learn how to find your <a href="https://www.paypal.com/us/cgi-bin/webscr?cmd=xpt/cps/merchant/wppro/WPProIntegrationSteps-outside#SectionB">API Signature</a>.', 'event_espresso'); ?>
</li>
<li>
<?php _e('Country Currency', 'event_espresso'); ?><br />
<?php _e('Select the currency for your country. Payments will be accepted in this currency.', 'event_espresso'); ?>
</li>
<li>
<?php _e('Accepted Credit Cards', 'event_espresso'); ?><br />
<?php _e('Select the card types that you want to accept payments from.', 'event_espresso'); ?>
</li>
<li>
<?php _e('Use the Debugging Feature and the PayPal Sandbox', 'event_espresso'); ?><br />
<?php _e('Specify if you want to test the payment gateway by submitting a test transaction. If this option is enabled, be sure to enter your PayPal sandbox credentials in the fields above. Be sure to turn this setting off when you are done testing.', 'event_espresso'); ?>
</li>
<li>
<?php _e('Button Image URL', 'event_espresso'); ?><br />
<?php _e('Change the image that is used for this payment gateway.', 'event_espresso'); ?>
</li>
</ul>