...
class Registration_Form_Questions_Admin_List_Table extends EE_Admin_List_Table {
	...
	public function column_cb(EE_Question $item) {
		...
		return $system_question || (!$system_question && $related_answer_count > 0 && $this->_view == 'trash' ) ? '<span class="ee-lock-icon' . $extra_lock_icon . '"></span>' . sprintf( '<input type="hidden" name="checkbox[%1$d]" value="%1$d" />', $item->ID() )  : sprintf( '<input type="checkbox" class="QST_ID" name="checkbox[%1$d]" value="%1$d" />', $item->ID() );
	}
