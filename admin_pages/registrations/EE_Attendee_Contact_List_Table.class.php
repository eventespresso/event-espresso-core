...
class EE_Attendee_Contact_List_Table extends EE_Admin_List_Table {
	...
	function column_ATT_lname($item) {
		...
		$name_link = EE_Registry::instance()->CAP->current_user_can( 'edit_contacts', 'espresso_registrations_edit_attendee' ) ?  '<a href="'.$edit_lnk_url.'" title="' . __( 'Edit Contact', 'event_espresso' ) . '">' . $item->lname() . '</a>' : $item->lname();
		return $name_link;
	...
	function column_ATT_fname($item) {
		...
		$actions = array();
		// edit attendee link
		if ( EE_Registry::instance()->CAP->current_user_can( 'edit_contacts', 'espresso_registrations_edit_attendee' ) ) {
			$edit_lnk_url = EE_Admin_Page::add_query_args_and_nonce( array( 'action'=>'edit_attendee', 'post'=>$item->ID() ), REG_ADMIN_URL );
		...
		if ( $this->_view == 'in_use' ) {
			...
			if ( EE_Registry::instance()->CAP->current_user_can( 'delete_contacts', 'espresso_registrations_restore_attendees' ) ) {
		...
		$edit_lnk_url = EE_Admin_Page::add_query_args_and_nonce( array( 'action'=>'edit_attendee', 'post'=>$item->ID() ), REG_ADMIN_URL );
		$name_link = EE_Registry::instance()->CAP->current_user_can( 'edit_contacts', 'espresso_registrations_edit_attendee' ) ?  '<a href="'.$edit_lnk_url.'" title="' . __( 'Edit Contact', 'event_espresso' ) . '">' . $item->fname() . '</a>' : $item->fname();
