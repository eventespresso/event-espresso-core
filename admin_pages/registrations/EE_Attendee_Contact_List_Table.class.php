...
class EE_Attendee_Contact_List_Table extends EE_Admin_List_Table
{
    ...
    {
        $this->_data = $this->_view != 'trash' ? $this->_admin_page->get_attendees($this->_per_page) : $this->_admin_page->get_attendees($this->_per_page,
            false, true);
        $this->_all_data_count = $this->_view != 'trash' ? $this->_admin_page->get_attendees($this->_per_page,
            true) : $this->_admin_page->get_attendees($this->_per_page, true, true);
    }
    ...
    {
        return sprintf('<input type="checkbox" name="checkbox[%1$s]" value="%1$s" />', /* $1%s */
            $item->ID());
    }
    ...
    function column_ATT_lname($item)
    {
        ...
        $name_link = EE_Registry::instance()->CAP->current_user_can('ee_edit_contacts',
            'espresso_registrations_edit_attendee') ? '<a href="' . $edit_lnk_url . '" title="' . esc_attr__('Edit Contact',
                'event_espresso') . '">' . $item->lname() . '</a>' : $item->lname();
        return $name_link;
    ...
    function column_ATT_fname($item)
    {
        ...
        $actions = array();
        // edit attendee link
        if (EE_Registry::instance()->CAP->current_user_can('ee_edit_contacts',
            'espresso_registrations_edit_attendee')) {
            $edit_lnk_url = EE_Admin_Page::add_query_args_and_nonce(array(
                'action' => 'edit_attendee',
                'post'   => $item->ID(),
            ), REG_ADMIN_URL);
            $actions['edit'] = '<a href="' . $edit_lnk_url . '" title="' . esc_attr__('Edit Contact',
        ...
        if ($this->_view == 'in_use') {
            ...
            if (EE_Registry::instance()->CAP->current_user_can('ee_delete_contacts',
                ...
                $actions['trash'] = '<a href="' . $trash_lnk_url . '" title="' . esc_attr__('Move Contact to Trash',
            ...
            if (EE_Registry::instance()->CAP->current_user_can('ee_delete_contacts',
                ...
                $actions['restore'] = '<a href="' . $restore_lnk_url . '" title="' . esc_attr__('Restore Contact',
        ...
        $name_link = EE_Registry::instance()->CAP->current_user_can('ee_edit_contacts',
            'espresso_registrations_edit_attendee') ? '<a href="' . $edit_lnk_url . '" title="' . esc_attr__('Edit Contact',
                'event_espresso') . '">' . $item->fname() . '</a>' : $item->fname();
