...
class Registrations_Admin_Page extends EE_Admin_Page_CPT
{
    ...
    {
        EE_Registry::$i18n_js_strings['update_att_qstns'] = esc_html__('click "Update Registration Questions" to save your changes',
            'event_espresso');
    }
    ...
    public function load_scripts_styles_edit_attendee()
    {
        ...
        $attendee_details_translations = array(
            'att_publish_text' => sprintf(esc_html__('Created on: <b>%1$s</b>', 'event_espresso'),
    ...
    protected function _set_list_table_views_default()
    {
        ...
        $def_reg_status_actions['approve_registration'] = esc_html__('Approve Registrations', 'event_espresso');
        if (in_array($match_array['approve_registration'], $active_mts)
            ...
            $def_reg_status_actions['approve_and_notify_registration'] = esc_html__('Approve and Notify Registrations',
                'event_espresso');
        }
        $def_reg_status_actions['decline_registration'] = esc_html__('Decline Registrations', 'event_espresso');
        if (in_array($match_array['decline_registration'], $active_mts)
            && EE_Registry::instance()->CAP->current_user_can('ee_send_message', 'batch_send_messages')
        ) {
            $def_reg_status_actions['decline_and_notify_registration'] = esc_html__('Decline and Notify Registrations',
                'event_espresso');
        }
        $def_reg_status_actions['pending_registration'] = esc_html__('Set Registrations to Pending Payment', 'event_espresso');
        if (in_array($match_array['pending_registration'], $active_mts)
            && EE_Registry::instance()->CAP->current_user_can('ee_send_message', 'batch_send_messages')
        ) {
            $def_reg_status_actions['pending_and_notify_registration'] = esc_html__('Set Registrations to Pending Payment and Notify',
                'event_espresso');
        }
        $def_reg_status_actions['no_approve_registration'] = esc_html__('Set Registrations to Not Approved', 'event_espresso');
        if (in_array($match_array['no_approve_registration'], $active_mts)
            && EE_Registry::instance()->CAP->current_user_can('ee_send_message', 'batch_send_messages')
        ) {
            $def_reg_status_actions['no_approve_and_notify_registration'] = esc_html__('Set Registrations to Not Approved and Notify',
                'event_espresso');
        }
        $def_reg_status_actions['cancel_registration'] = esc_html__('Cancel Registrations', 'event_espresso');
        if (in_array($match_array['cancel_registration'], $active_mts)
            && EE_Registry::instance()->CAP->current_user_can('ee_send_message', 'batch_send_messages')
        ) {
            $def_reg_status_actions['cancel_and_notify_registration'] = esc_html__('Cancel Registrations and Notify',
                'event_espresso');
        }
        $this->_views = array(
            'all'   => array(
            ...
            'today' => array(
                'slug'        => 'today',
                'label'       => sprintf(esc_html__('Today - %s', 'event_espresso'), date('M d, Y', current_time('timestamp'))),
                'count'       => 0,
                'bulk_action' => array_merge($def_reg_status_actions, array(
                    'trash_registrations' => esc_html__('Trash Registrations', 'event_espresso'),
                )),
    ...
    protected function _registrations_overview_list_table()
    {
        ...
        if ($EVT_ID) {
            ...
            if ($event instanceof EE_Event) {
                $this->_template_args['admin_page_header'] = sprintf(esc_html__('%s Viewing registrations for the event: %s%s',
    ...
    private function _set_registration_object()
    {
        ...
        if ($this->_registration = $REG->get_one_by_ID($REG_ID)) {
            ...
            $error_msg = sprintf(esc_html__('An error occurred and the details for Registration ID #%s could not be retrieved.',
    ...
    protected function _registration_details_metaboxes()
    {
        ...
        if ($attendee instanceof EE_Attendee
            ...
            add_meta_box('edit-reg-questions-mbox', esc_html__('Registration Form Answers', 'event_espresso'),
                array($this, '_reg_questions_meta_box'), $this->wp_page_slug, 'normal', 'high');
        }
        add_meta_box('edit-reg-registrant-mbox', esc_html__('Contact Details', 'event_espresso'),
            array($this, '_reg_registrant_side_meta_box'), $this->wp_page_slug, 'side', 'high');
        if ($this->_registration->group_size() > 1) {
            add_meta_box('edit-reg-attendees-mbox', esc_html__('Other Registrations in this Transaction', 'event_espresso'),
    ...
    protected function _generate_reg_status_change_form()
    {
        return new EE_Form_Section_Proper(array(
            ...
            'subsections'     => array(
                ...
                'current_status'     => new EE_Form_Section_HTML(EEH_HTML::tr(EEH_HTML::th(EEH_HTML::label(EEH_HTML::strong(esc_html__('Current Registration Status',
                ...
                'send_notifications' => new EE_Yes_No_Input(array(
                    ...
                    'default'         => false,
                    'html_help_text'  => esc_html__('If set to "Yes", then the related messages will be sent to the registrant.',
                        'event_espresso'),
                )),
    ...
    protected function _reg_status_change_return($STS_ID, $notify = false)
    {
        ...
        if ($success) {
            ...
            EE_Error::add_error(esc_html__('Something went wrong, and the status was not changed', 'event_espresso'), __FILE__,
    ...
    public function _reg_details_meta_box()
    {
        ...
        if (EE_Registry::instance()->CAP->current_user_can('ee_read_transaction',
            ...
            ), TXN_ADMIN_URL), esc_htmlesc_html__(' View Transaction'), 'button secondary-button right',
        ...
        if ($attendee instanceof EE_Attendee
            ...
            ), REG_ADMIN_URL), esc_htmlesc_html__(' Resend Registration'), 'button secondary-button right',
        ...
        if (isset($reg_details['registration_id'])) {
            ...
            $this->_template_args['reg_details']['registration_id']['label'] = esc_html__('Registration ID', 'event_espresso');
            $this->_template_args['reg_details']['registration_id']['class'] = 'regular-text';
        }
        if (isset($reg_details['payment_method'])) {
            $this->_template_args['reg_details']['payment_method']['value'] = $reg_details['payment_method'];
            $this->_template_args['reg_details']['payment_method']['label'] = esc_html__('Most Recent Payment Method',
                'event_espresso');
            $this->_template_args['reg_details']['payment_method']['class'] = 'regular-text';
            $this->_template_args['reg_details']['response_msg']['value']   = $reg_details['response_msg'];
            $this->_template_args['reg_details']['response_msg']['label']   = esc_html__('Payment method response',
                'event_espresso');
            $this->_template_args['reg_details']['response_msg']['class']   = 'regular-text';
        }
        $this->_template_args['reg_details']['registration_session']['value'] = $reg_details['registration_session'];
        $this->_template_args['reg_details']['registration_session']['label'] = esc_html__('Registration Session',
            'event_espresso');
        $this->_template_args['reg_details']['registration_session']['class'] = 'regular-text';
        $this->_template_args['reg_details']['ip_address']['value']           = $reg_details['ip_address'];
        $this->_template_args['reg_details']['ip_address']['label']           = esc_html__('Registration placed from IP',
            'event_espresso');
    ...
    public function form_before_question_group($output)
    {
        EE_Error::doing_it_wrong(__CLASS__ . '::' . __FUNCTION__,
            esc_html__('This method would have been protected but was used on a filter callback'
    ...
    public function form_after_question_group($output)
    {
        ...
        return '
			<tr class="hide-if-no-js">
               ...
               . esc_attresc_html__('click to edit question', 'event_espresso')
               . '">
...
';
    ...
    {
        if ( ! $REG_ID) {
            EE_Error::add_error(esc_html__('An error occurred. No registration ID was received.', 'event_espresso'), __FILE__,
    ...
    public function _reg_attendees_meta_box()
    {
        ...
        if (empty($registrations)
            ...
            EE_Error::add_error(esc_html__('There are no records attached to this registration. Something may have gone wrong with the registration',
    ...
    protected function _trash_or_restore_registrations($trash = true)
    {
        ...
        $this->_redirect_after_action(
            ...
            $success > 1 ? esc_html__('Registrations', 'event_espresso') : esc_html__('Registration', 'event_espresso'),
            $trash ? esc_html__('moved to the trash', 'event_espresso') : esc_html__('restored', 'event_espresso'),
            array('action' => 'default'),
            $overwrite_msgs
        );
    }
    ...
    protected function _delete_registrations()
    {
        ...
        $what        = $success > 1 ? esc_html__('Registrations', 'event_espresso') : esc_html__('Registration', 'event_espresso');
    ...
    protected function _delete_registration(EE_Registration $REG)
    {
        ...
        if ( ! $all_trashed) {
            EE_Error::add_error(esc_html__('Unable to permanently delete this registration. Before this registration can be permanently deleted, all registrations made in the same transaction must be trashed as well.  These registrations will be permanently deleted in the same action.',
                'event_espresso'), __FILE__, __FUNCTION__, __LINE__);
            return false;
        }
    ...
    public function new_registration()
    {
        if ( ! $this->_set_reg_event()) {
            throw new EE_Error(esc_html__('Unable to continue with registering because there is no Event ID in the request',
                'event_espresso'));
        }
    ...
    protected function _get_registration_step_content()
    {
        if (isset($_COOKIE['ee_registration_added']) && $_COOKIE['ee_registration_added']) {
            $warning_msg = sprintf(esc_html__('%2$sWARNING!!!%3$s%1$sPlease do not use the back button to return to this page for the purpose of adding another registration.%1$sThis can result in lost and/or corrupted data.%1$sIf you wish to add another registration, then please click the%1$s%7$s"Add Another New Registration to Event"%8$s button%1$son the Transaction details page, after you are redirected.%1$s%1$s%4$s redirecting in %5$s seconds %6$s',
	...
	<script >
        ...
        switch ($step) {
            case 'ticket' :
                ...
                $template_args['title']                            = esc_html__('Step One: Select the Ticket for this registration',
                    'event_espresso');
                $template_args['content']                          = EED_Ticket_Selector::instance()->display_ticket_selector($this->_reg_event);
                $template_args['step_button_text']                 = esc_html__('Add Tickets and Continue to Registrant Details',
                    'event_espresso');
                $template_args['show_notification_toggle']         = false;
                break;
            case 'questions' :
                $hidden_fields['processing_registration']['value'] = 2;
                $template_args['title']                            = esc_html__('Step Two: Add Registrant Details for this Registration',
                    'event_espresso');
                ...
                $template_args['step_button_text']         = esc_html__('Save Registration and Continue to Details',
                    'event_espresso');
    ...
    public function process_reg_step()
    {
        ...
        switch ($step) {
            case 'ticket' :
                ...
                if ($success) {
                    EE_Error::add_success(esc_htmlesc_html__('Tickets Selected. Now complete the registration.',
                        'event_espresso'));
                } else {
    ...
    public function redirect_to_txn()
    {
        ...
        EE_Error::add_success(esc_html__('Registration Created.  Please review the transaction and add any payments as necessary',
            'event_espresso'));
        $this->_redirect_after_action(false, '', '', $query_args, true);
    }
    ...
    protected function _duplicate_attendee()
    {
        ...
        if (empty($this->_req_data['_REG_ID'])) {
            EE_Error::add_error(esc_html__('Unable to create the contact for the registration because the required parameters are not present (_REG_ID )',
        ...
        EE_Error::add_success(esc_html__('New Contact record created.  Now make any edits you wish to make for this contact.',
            'event_espresso'));
        ...
        if ($post->post_type = 'espresso_attendees' && ! empty($attendee)) {
            ...
            foreach ($attendee_update_callbacks as $a_callback) {
                if (false === call_user_func_array($a_callback, array($attendee, $this->_req_data))) {
                    throw new EE_Error(sprintf(esc_html__('The %s callback given for the "FHEE__Registrations_Admin_Page__insert_update_cpt_item__attendee_update" filter is not a valid callback.  Please check the spelling.',
                        'event_espresso'), $a_callback));
                }
            }
        }
        if ($success === false) {
            EE_Error::add_error(esc_html__('Something went wrong with updating the meta table data for the registration.',
                'event_espresso'), __FILE__, __FUNCTION__, __LINE__);
        }
    }
    ...
    public function attendee_editor_metaboxes()
    {
        ...
        remove_meta_box('postexcerpt', esc_html__('Excerpt'), 'post_excerpt_meta_box', $this->_cpt_routes[$this->_req_action],
            'normal', 'core');
        remove_meta_box('commentstatusdiv', $this->_cpt_routes[$this->_req_action], 'normal', 'core');
        if (post_type_supports('espresso_attendees', 'excerpt')) {
            add_meta_box('postexcerpt', esc_html__('Short Biography', 'event_espresso'), 'post_excerpt_meta_box',
                $this->_cpt_routes[$this->_req_action], 'normal');
        }
        if (post_type_supports('espresso_attendees', 'comments')) {
            add_meta_box('commentsdiv', esc_html__('Notes on the Contact', 'event_espresso'), 'post_comment_meta_box',
                $this->_cpt_routes[$this->_req_action], 'normal', 'core');
        }
        add_meta_box('attendee_contact_info', esc_html__('Contact Info', 'event_espresso'),
            array($this, 'attendee_contact_info'), $this->_cpt_routes[$this->_req_action], 'side', 'core');
        add_meta_box('attendee_details_address', esc_html__('Address Details', 'event_espresso'),
            array($this, 'attendee_address_details'), $this->_cpt_routes[$this->_req_action], 'normal', 'core');
        add_meta_box('attendee_registrations', esc_html__('Registrations for this Contact', 'event_espresso'),
    ...
    protected function _trash_or_restore_attendees($trash = true)
    {
        ...
        $what        = $success > 1 ? esc_html__('Contacts', 'event_espresso') : esc_html__('Contact', 'event_espresso');
        $action_desc = $trash ? esc_html__('moved to the trash', 'event_espresso') : esc_html__('restored', 'event_espresso');
