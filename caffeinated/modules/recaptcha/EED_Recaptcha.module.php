...
class EED_Recaptcha extends EED_Module
{
    ...
    public static function set_hooks_admin()
    {
        ...
        if (
            ...
            EED_Recaptcha::enqueue_styles_and_scripts();
            add_filter('FHEE__EED_Single_Page_Checkout__init___continue_reg', array('EED_Recaptcha', 'not_a_robot'),
                10);
            add_filter('FHEE__EE_SPCO_Reg_Step__set_completed___completed', array('EED_Recaptcha', 'not_a_robot'), 10);
            add_filter('FHEE__EE_SPCO_JSON_Response___toString__JSON_response',
                array('EED_Recaptcha', 'recaptcha_response'), 10, 1);
        }
        // admin settings
        add_action('AHEE__Extend_Registration_Form_Admin_Page___reg_form_settings_template',
            array('EED_Recaptcha', 'admin_settings'), 10, 1);
        add_filter('FHEE__Extend_Registration_Form_Admin_Page___update_reg_form_settings__CFG_registration',
            array('EED_Recaptcha', 'update_admin_settings'), 10, 1);
    }
    ...
    {
        wp_register_script('espresso_recaptcha', RECAPTCHA_BASE_URL . 'scripts' . DS . 'espresso_recaptcha.js',
            array('single_page_checkout'), EVENT_ESPRESSO_VERSION, true);
        wp_register_script('google_recaptcha', 'https://www.google.com/recaptcha/api.js?hl='
                                               . EE_Registry::instance()->CFG->registration->recaptcha_language,
            array('espresso_recaptcha'), EVENT_ESPRESSO_VERSION, true);
        EE_Registry::$i18n_js_strings['no_SPCO_error']      = __('It appears the Single Page Checkout javascript was not loaded properly! Please refresh the page and try again or contact support.',
            'event_espresso');
        EE_Registry::$i18n_js_strings['no_recaptcha_error'] = __('There appears to be a problem with the reCAPTCHA configuration! Please check the admin settings or contact support.',
            'event_espresso');
        EE_Registry::$i18n_js_strings['recaptcha_fail']     = __('Please complete the anti-spam test before proceeding.',
            'event_espresso');
    }
    ...
    private static function _bypass_recaptcha()
    {
        ...
        $bypass_request_params_array = apply_filters('FHEE__EED_Recaptcha___bypass_recaptcha__bypass_request_params_array',
            array());
        ...
        return new EE_Form_Section_Proper(
            array(
                ...
                'subsections'     => apply_filters(
                    ...
                    array(
                        'main_settings_hdr'       => new EE_Form_Section_HTML(EEH_HTML::h2(__('reCAPTCHA Anti-spam Settings',
                                'event_espresso') . EEH_Template::get_help_tab_link('recaptcha_info'))),
                        'main_settings'           => EED_Recaptcha::_recaptcha_main_settings(),
                        'appearance_settings_hdr' => new EE_Form_Section_HTML(EEH_HTML::h2(__('reCAPTCHA Appearance',
                            'event_espresso'))),
                        ...
                        'required_fields_note'    => new EE_Form_Section_HTML(EEH_HTML::p(__('All fields marked with a * are required fields',
                            'event_espresso'), '', 'grey-text')),
                    )
                ),
            )
        );
                    ...
                    array(
                        'use_captcha'          => new EE_Yes_No_Input(
                            array(
                                ...
                                'html_help_text'          => sprintf(
                                    __('reCAPTCHA is a free service that  protects your website from spam and abuse. It employs advanced risk analysis technology to separate humans from abusive actors. Sign up %1$shere%2$s to receive your Public and Private keys.',
                                        'event_espresso'),
                                    '<a href="https://www.google.com/recaptcha/intro/index.html">',
                                    '</a>'
                                ),
                                'default'                 => isset(EE_Registry::instance()->CFG->registration->use_captcha)
                                    ? EE_Registry::instance()->CFG->registration->use_captcha : false,
                        ...
                        'recaptcha_publickey'  => new EE_Text_Input(
                            array(
                                ...
                                'html_help_text'  => __('The site key is used to display the widget on your site.',
                                    'event_espresso'),
                                'default'         => isset(EE_Registry::instance()->CFG->registration->recaptcha_publickey)
                                    ? stripslashes(EE_Registry::instance()->CFG->registration->recaptcha_publickey) : '',
                            )
                        ),
                        'recaptcha_privatekey' => new EE_Text_Input(
                            array(
                                'html_label_text' => __('Secret Key', 'event_espresso'),
                                'html_help_text'  => __('The secret key authorizes communication between your application backend and the reCAPTCHA server to verify the user\'s response. The secret key needs to be kept safe for security purposes.',
                                    'event_espresso'),
                                'default'         => isset(EE_Registry::instance()->CFG->registration->recaptcha_privatekey)
                                    ? stripslashes(EE_Registry::instance()->CFG->registration->recaptcha_privatekey)
                                    : '',
                            )
                        ),
                    )
                    ...
                    array(
                        'recaptcha_theme'    => new EE_Radio_Button_Input(
                            ...
                            array(
                                ...
                                'default'                 => isset(EE_Registry::instance()->CFG->registration->recaptcha_theme)
                                    ? EE_Registry::instance()->CFG->registration->recaptcha_theme : 'light',
                                'display_html_label_text' => false,
                            )
                        ),
                        'recaptcha_type'     => new EE_Radio_Button_Input(
                            ...
                            array(
                                ...
                                'default'                 => isset(EE_Registry::instance()->CFG->registration->recaptcha_type)
                                    ? EE_Registry::instance()->CFG->registration->recaptcha_type : 'image',
                                'display_html_label_text' => false,
                            )
                        ),
                        'recaptcha_language' => new EE_Select_Input(
                            ...
                            array(
                                ...
                                'html_help_text'  => __('Forces the widget to render in a specific language.',
                                    'event_espresso'),
                                'default'         => isset(EE_Registry::instance()->CFG->registration->recaptcha_language)
                                    ? EE_Registry::instance()->CFG->registration->recaptcha_language : 'en',
                            )
                        ),
                    )
                ),
        ...
        try {
            ...
            if ($recaptcha_settings_form->was_submitted()) {
                ...
                if ($recaptcha_settings_form->is_valid()) {
                    ...
                    if (
                        apply_filters('FHEE__Extend_Registration_Form_Admin_Page__check_for_recaptcha_keys', true,
                        ...
                        EE_Error::add_error(__('The use reCAPTCHA setting has been reset to "no". In order to enable the reCAPTCHA service, you must enter a Site Key and Secret Key.',
                    ...
                    if ($recaptcha_settings_form->submission_error_message() != '') {
                        EE_Error::add_error($recaptcha_settings_form->submission_error_message(), __FILE__,
