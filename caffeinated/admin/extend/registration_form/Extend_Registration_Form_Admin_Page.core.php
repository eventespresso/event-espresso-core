...
*@param EEM_Soft_Delete_Base $model
	...
	protected function _reg_form_settings() {
		...
		add_action(
			'AHEE__Extend_Registration_Form_Admin_Page___reg_form_settings_template',
			array( $this, 'invalid_checkout_access_form' ),
			15
		);
	 ...
	 * @throws \EE_Error
	 */
	public function invalid_checkout_access_form() {
		echo $this->_invalid_checkout_access_form()->get_html();
	}
	 ...
	 * @throws \EE_Error
	 */
	protected function _invalid_checkout_access_form() {
		return new EE_Form_Section_Proper(
			array(
				...
				'subsections'     => array(
					...
					'ee_bot_checkout_data' => new EE_Text_Area_Input(
						array(
							...
							'default'         => var_export( get_option( EED_Single_Page_Checkout::ICA_OPTION_KEY, array() ), true ),
							'required'        => false,
	 ...
	 * @return \EE_Registration_Config
	 */
	public function update_invalid_checkout_access_form( EE_Registration_Config $EE_Registration_Config ) {
		try {
			$invalid_checkout_access_form = $this->_invalid_checkout_access_form();
			// if not displaying a form, then check for form submission
				...
				if ( $invalid_checkout_access_form->is_valid() ) {
					...
					if (
						...
						if ( filter_var( $valid_data['delete_invalid_checkout_data'], FILTER_VALIDATE_BOOLEAN ) ) {
							update_option( EED_Single_Page_Checkout::ICA_OPTION_KEY, array() );
						}
					} else {
						EE_Error::add_error(
							esc_html__(
