...
class Price_Types_List_Table extends EE_Admin_List_Table {
	...
	function column_name($item) {
		...
		$actions = array();
		// edit price link
		if ( EE_Registry::instance()->CAP->current_user_can( 'edit_default_price_type', 'edit_price_type', $item->ID() ) ) {
		...
		$name_link = EE_Registry::instance()->CAP->current_user_can( 'edit_default_price_type', 'edit_price_type', $item->ID() ) ? '<a href="'.$edit_lnk_url.'" title="' . __( 'Edit Price Type', 'event_espresso' ) . '">' . stripslashes( $item->name() ) . '</a>' : $item->name();

		if ( $item->base_type() !== 1 ) {
			if ( $this->_view == 'all' ) {
				// trash price link
				if ( EE_Registry::instance()->CAP->current_user_can( 'delete_default_price_type', 'trash_price_type', $item->ID() ) ) {
				...
				// restore price link
				if ( EE_Registry::instance()->CAP->current_user_can( 'delete_default_price_type', 'restore_price_type', $item->ID() ) ) {
				...
				// delete price link
				if ( EE_Registry::instance()->CAP->current_user_can( 'delete_default_price_type', 'delete_price_type', $item->ID() ) ) {
