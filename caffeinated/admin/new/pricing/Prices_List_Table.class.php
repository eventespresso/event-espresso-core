...
class Prices_List_Table extends EE_Admin_List_Table {
	...
	function column_name($item) {
		...
		$actions = array();
		// edit price link
		if ( EE_Registry::instance()->CAP->current_user_can( 'edit_default_price', 'edit_price', $item->ID() ) ) {
		...
		if ( $item->type_obj()->base_type() !== 1 ) {
			if ($this->_view == 'all') {
				// trash price link
				if ( EE_Registry::instance()->CAP->current_user_can( 'delete_default_price', 'trash_price', $item->ID() ) ) {
				...
				if ( EE_Registry::instance()->CAP->current_user_can( 'delete_default_price', 'restore_price', $item->ID() ) ) {
					// restore price link
				...
				// delete price link
				if ( EE_Registry::instance()->CAP->current_user_can( 'delete_default_price', 'delete_price', $item->ID() ) ) {
