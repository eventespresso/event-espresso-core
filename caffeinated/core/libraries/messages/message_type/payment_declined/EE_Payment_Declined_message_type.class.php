...
class EE_Payment_Declined_message_type extends EE_message_type {
	...
	protected function _default_template_field_content() {
		$content = file_get_contents( EE_CAF_LIBRARIES . 'messages/message_type/payment_declined/templates/payment-declined-message-type-content.template.php');

		foreach ( $this->_contexts as $context => $details ) {
			$tcontent[$context]['main'] = $content;
			$tcontent[$context]['event_list'] = file_get_contents( EE_CAF_LIBRARIES . 'messages/message_type/payment_declined/templates/payment-declined-message-type-event-list.template.php');
