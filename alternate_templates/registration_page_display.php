		...
		<form method="post" action="<?php echo home_url() ?>/?page_id=<?php echo $event_page_id ?>" id="registration_form">
			...
			switch ($is_active['status']) {
				...
				default://This will display the registration form
					...
					if ($show_ee_gmap_no_shortcode && $event->is_enable_for_gmap() !== 'N') {
					...
					if ($event->is_display_reg_form() == 'Y') {
						...
						<fieldset id="event-reg-form-groups" class="ui-widget">
							...
							echo espresso_display_questions($questions, 'primary');
							?>
							<div class="event_prices event_questions">
								<?php echo espresso_price_selection($event, 'primary'); ?>
							</div>
						</fieldset>
