...
describe('useTimeZoneTime', () => {
	...
	describe('localTimeToUtc', () => {
		it('returns UTC date for a given local date as ISO string', () => {
			const testDate = '2020-01-28T13:00:20'; // IST
			const expectedDate = '2020-01-28T07:30:20.000Z'; // 5:30 ahead of UTC
			const {
				result: { current: TZ },
		...
		it('returns UTC date for a given local date as Date insatance', () => {
			const testDate = new Date(2020, 0 /* Jan */, 28, 13, 0, 20, 0); // IST
			const expectedDate = '2020-01-28T07:30:20.000Z'; // 5:30 ahead of UTC
			const {
				result: { current: TZ },
			} = renderHook(() => useTimeZoneTime(), { wrapper });
	...
	describe('utcToLocalTime', () => {
		it('returns local date for a given UTC date as ISO string', () => {
			const testDate = '2020-01-28T07:30:20'; // IST
			const expectedDate = '2020-01-28T02:00:20.000Z'; // 5:30 behind IST
			const {
				result: { current: TZ },
			} = renderHook(() => useTimeZoneTime(), { wrapper });
		...
		it('returns local date for a given UTC date as Date insatance', () => {
			const testDate = new Date(2020, 0 /* Jan */, 28, 7, 30, 20, 0); // IST
			const expectedDate = '2020-01-28T02:00:20.000Z'; // 5:30 behind IST
			const {
				result: { current: TZ },
			} = renderHook(() => useTimeZoneTime(), { wrapper });
