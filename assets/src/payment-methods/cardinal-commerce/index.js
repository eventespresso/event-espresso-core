...
jQuery( document ).ready( () => {
	...
	eeCardinalCruise = new EeCardinalCruise(
		eeCardinalCommerceData.data,
		eeCardinalCommerceData.translations,
	);
...
function EeCardinalCruise( instanceVars, translations ) {
	...
	this.setInitListeners = function() {
		...
		//also, if the page was reloaded on the payment option step, we should
	...
	/**
	 * When SPCO displays a step, if its the payment options step, and our
	 * billing form is present, initialize the smart buttons
	 *
	 */
	this.setListenerForDisplaySpco = function() {
		this.spco.main_container.on(
			'spco_display_step',
			( event, stepToShow ) => {
				if (
					typeof stepToShow !== 'undefined' &&
	 ...
	 * initialize the smart button (or if it's already initialized, just show
	 * it again). If they selected a different payment method, hide the smart
	 * buttons
	 */
	this.setListenerForPaymentMethodSelector = function() {
		this.spco.main_container.on(
			'spco_switch_payment_methods',
			( event, paymentMethod ) => {
				if (
					typeof paymentMethod !== 'undefined' &&
					paymentMethod === this.slug
				) {
					this.initialize();
				} else if ( this.initialized ) {
					//and if this was previously initialized, make sure we hide
					// the button
				}
			},
		);
	 ...
	 * @return {boolean} whether it was successfully loaded or not.
	 */
	...
	/**
	 * Initializes jQuery selected objects so we don't need to query for
	 * anything afterwards
	 */
	this.initializeObjects = function() {
		// Only initialize objects once. It's not removed when switching
		// payment methods, and PayPal's JS malfunctions if this gets reset.
		if ( ! this.initialized ) {
		}
	};

	/**
	 * Shows the smart buttons (this may require initializing them) and
	 * otherwise initializes this object
	 */
	this.initialize = function() {
		if (
			typeof this.spco === 'undefined' ||
			typeof this.spco.show_event_queue_ajax_msg !== 'function' ||
			typeof this.spco.display_messages !== 'function' ||
			! this.spco.main_container
		) {
			// No SPCO object, so we can't use SPCO to show a nice error
			// message. At least put something in the console.
			warning( false, this.translations.no_SPCO_error );
			return;
		}
		// ensure that the Cardinal Commerce object (from
		// https://songbird.cardinalcommerce.com/edge/v1/songbird.js) js class
		// is loaded
		if ( typeof this.cardinal === 'undefined' ) {
			...
			this.spco.show_event_queue_ajax_msg(
				'error',
				this.translations.no_cardinal_error,
				this.spco.notice_fadeout_attention,
				true,
			);
			return;
		}
		...
		// Ran when Cardinal is ready to go. See
		// https://cardinaldocs.atlassian.net/wiki/spaces/CC/pages/557065/Songbird.js#Songbird.js-payments.setupComplete
		...
		// Always re-initialize jQuery objects. If they were payment method
		// switching, the old billing for inputs got removed from the page and
		// we need to find them again.
		this.initializeObjects();
	};

	/**
	 * Ran when Cardinal Cruise has finished validating  the mode of payment.
	 * Continue with processing the payment.
	 */
	this.setListenerForCardinalPaymentValidated = function() {
		this.cardinal.on( 'payments.validated', ( paymentData, jwt ) => {
			if ( typeof paymentData !== 'object' ) {
				this.spco.show_event_queue_ajax_msg(
					'error',
					this.translations.invalid_response_from_cardinal,
					this.spco.notice_fadeout_attention,
					true,
				);
				return;
			}
			if ( paymentData.ActionCode === 'ERROR' ) {
				this.spco.show_event_queue_ajax_msg(
					'error',
					paymentData.ErrorDescription + '[' + paymentData.ErrorNumber + ']',
					this.spco.notice_fadeout_attention,
					true,
				);
				return;
			...
			this.saveCardinalCruiseResult( paymentData, jwt );
	...
	this.saveCardinalCruiseResult = function( ccData, jwt ) {
	...
	/**
	 * When the payment amount changes, just update this object's
	 * transaction_total
	 */
				...
				Consumer: {
					Account: {
						AccountNumber: jQuery(
							'#paypal-pro-billing-form-credit-card' ).val(),
						ExpirationMonth: jQuery(
							'#paypal-pro-billing-form-exp-month' ).val(),
						ExpirationYear: jQuery(
							'#paypal-pro-billing-form-exp-year' ).val(),
						CardCode: jQuery( '#paypal-pro-billing-form-cvv' ).val(),
					},
				},
			};
			alert( 'Cardinal sent credit card details off I think' );
			this.cardinal.start( 'cca', billingData, this.jwt );
		} );
	};

	this.getForm = function() {
		if ( this.billingForm === null ) {
			this.billingForm = jQuery( '#paypal-pro-billing-form-credit-card' )
				.parents( 'form:first' );
		}
