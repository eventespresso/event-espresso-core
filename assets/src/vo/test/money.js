...
describe( 'Money Value Object', () => {
	describe( 'Constructing Money Value Object', () => {
		...
		describe( 'hasSameCurrency()', () => {
			it( 'throws an error if invalid type provided for argument', () => {
				expect( () => testMoney.hasSameCurrency( {} ) ).toThrow(
					TypeError
				);
			} );
			it(
		...
		describe( 'allocate()', () => {
			it(
				...
				() => {
					const testComplexAllocation = testMoney.allocate( [
						1,
						1,
						1,
					] );
					expect( testComplexAllocation ).toHaveLength( 3 );
					expect( testComplexAllocation[ 0 ].toNumber() ).toBe(
						0.42
					);
					expect( testComplexAllocation[ 1 ].toNumber() ).toBe(
						0.42
					);
					expect( testComplexAllocation[ 2 ].toNumber() ).toBe(
						0.41
					);
				}
			);
		...
		describe( 'isZero()', () => {
			it( 'returns true if this object has a value of zero', () => {
				expect( testMoney.setAmount( new Decimal( 0 ) ).isZero() ).toBe(
					true
				);
			} );
			it(
		...
		describe( 'isNegative()', () => {
			it( 'returns true if this object has a negative value', () => {
				expect(
					testMoney.setAmount( new Decimal( -1 ) ).isNegative()
				).toBe( true );
			} );
		...
		describe( 'isPositive()', () => {
			...
			it( 'returns false if this object has a negative value', () => {
				expect(
					testMoney.setAmount( new Decimal( -1 ) ).isPositive()
				).toBe( false );
			} );
		} );
		describe( 'toNumber()', () => {
		...
		describe( 'toIntegerMoney()', () => {
			...
			it( 'returns expected new Money object for changed rounding', () => {
				...
				expect(
					differentRoundingMoney.toIntegerMoney().toNumber()
				).toBe( 3 );
				// return to default
	...
	describe( 'Money.assertSameCurrency()', () => {
		it( 'throws a TypeError if provided values are not currencies', () => {
			expect( () => Money.assertSameCurrency( {}, {} ) ).toThrow(
				TypeError
			);
		} );
	...
	describe( 'Money.fromMoneyValue', () => {
		it(
			...
			() => {
				expect( () => Money.fromMoneyValue( '10.10' ) ).toThrow(
					TypeError
				);
			}
		);
		it( 'converts a number as expected', () => {
			expect(
				Money.fromMoneyValue( 10, DefaultCurrency ).toNumber()
			).toBe( 10 );
			expect(
				Money.fromMoneyValue( 10.1, DefaultCurrency ).toNumber()
			).toBe( 10.1 );
		} );
		it(
		...
		it(
			...
			() => {
				expect( () =>
					Money.fromMoneyValue( '10,10€', DefaultCurrency )
				).toThrow();
			}
		);
	} );
