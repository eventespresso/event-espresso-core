	...
	describe('with Instance', () => {
					...
					baseUnits.forEach((unit) => {
						...
						describe(fluentSetter + '()', () => {
							...
							it('new DateTime has new Value for ' + unit, () => {
								expect(newDateTime[unit]).toBe(newValues[unit]);
							});
						});
					});
				});
						...
						describe('Valid instantiation tests', () => {
							const testDateTime =
								offsetValue && offsetTest
									...
									: DateTime[METHOD](dateConstructedWith());
				...
				describe(name + ' tests', () => {
					describe(name + ' accessor', () => {
						it('returns expected ' + name, () => {
							expect(testDate[name]).toBe(expectedOriginalValue);
						});
					});
					describe(setter + '()', () => {
		...
		describe('hasSame()', () => {
			...
			it('returns false when dates do not have same unit', () => {
				expect(
					baseTestDate.hasSame(hasSameHourDate, DateTime.UNIT_MINUTE)
				).toBe(false);
			});
		});
		describe('minus()', () => {
		...
		describe('toFormat', () => {
			...
			testConditions.forEach((testCondition) => {
				...
				it('returns expected value for the format: ' + format, () => {
					expect(baseTestDate.toFormat(format)).toBe(expectedResult);
				});
			});
		});
		describe('toISO()', () => {
		...
		describe('toLocal()', () => {
			...
			it('value for local date to be expected', () => {
				expect(localDate.toObject()).toEqual(expectedDate.toObject());
			});
		});
		describe('toMillis()', () => {
			it('returns expected value', () => {
