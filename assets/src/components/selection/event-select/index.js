...
const { Placeholder, SelectControl, withAPIData } = wp.components;
const { __ } = wp.i18n;

const nowDateAndTime = moment();

buildEventOptions = (events) => {
 ...
 * included as needed.
 * @type {{events: *, onEventSelect, selectLabel: *, selectedEventId: *, isLoading: *, attributes: {limit: *, orderBy: *, order: *, showExpired: *, categorySlug: *, month: *}}}
 */
EventSelect.propTypes = {
  ...
  attributes: PropTypes.shape({
    ...
    orderBy: PropTypes.oneOf(['EVT_name',
        'EVT_ID',
        'start_date',
        'end_date',
        'ticket_start',
 ...
 * things maybe?
 * @param orderBy
 * @returns {{start_date: string, end_date: string, ticket_start: string, ticket_end: string}}
 */
const mapOrderBy = (orderBy) => {
  ...
  return isUndefined(orderByMap[orderBy]) ? orderBy : orderByMap;
};
