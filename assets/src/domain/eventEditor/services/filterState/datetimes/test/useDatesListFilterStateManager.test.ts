...
import useDatesListFilterStateManager from './index';
...
describe('useDatesListFilterStateManager', () => {
	test('useDatesListFilterStateManager result', () => {
		const { result } = renderHook(() => useDatesListFilterStateManager(datetimes));
	...
	test('should update filteredEntities to reflect changes made by invoking setShowDates with recentlyExpiredOnly filter', () => {
		...
		const { result } = renderHook(() => useDatesListFilterStateManager(dates));
