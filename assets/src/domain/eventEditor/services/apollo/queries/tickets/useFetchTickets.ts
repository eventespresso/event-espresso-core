...
import { errorNotification, successNotification } from '../../../../../../application/services/notification';
...
const useFetchTickets = (skipFetch: boolean = null): FetchEntitiesResult<TicketsList> => {
	...
	const { data, error, loading } = useQuery<TicketsList>(query, {
		...
		onCompleted: (): void => {
			...
			successNotification({ message: `tickets initialized` });
		},
		onError: (error): void => {
			setIsError(TypeName.tickets, true);
			toaster.dismiss(toasterMessage);
			errorNotification({ message: `datetimes initialized` });
		},
	});
