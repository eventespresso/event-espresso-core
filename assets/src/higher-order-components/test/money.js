...
describe( 'withMoney()', () => {
	...
	it( 'Passes through props with no mapping provided', () => {
		const wrapper = renderer.create(
			getWrappedTestComponent( { propA: 'testa', propB: 'testb' } )
		);
	...
	it( 'Gives a warning if propNameMap is not valid', () => {
		...
		const expectedMessage =
			'Warning: The propNameMap argument provided to ' +
	...
	describe(
		'It gives a warning if propNameMap is a function that returns' +
			' unexpected shape',
		() => {
			const expectedNoValuesWarning =
				'Warning: The propNameMap callback ' +
				'for the withMoney HOC should return an object with a ' +
				'"convertedValues" key.';
			const expectedValuesNotArrayWarning =
				'Warning: The propNameMap' +
			...
			it(
				'gives warnings for when object contains a convertedValues property' +
	...
	it( 'Does not rerender unnecessarily', () => {
		...
		wrapper.update(
			<EnhancedComponent propA={ 2.25 } propB={ 1000.25 } />
		);
