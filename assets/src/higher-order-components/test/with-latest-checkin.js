...
describe( 'withLatestCheckin', () => {
	const getLatestCheckin = jest
		.fn()
		.mockImplementation( () => ( { checkIn: true } ) );
	...
	describe(
		'has expected props when value for registration is not an ' +
		...
		() => {
			...
			it( 'wrapped component has expected props', () => {
				...
				expect(
					testedComponent.props.hasResolvedCheckin
				).toBeUndefined();
	...
	describe(
		'has expected props when value for registration is an instance ' +
			'of BaseEntity',
		() => {
			let wrapper, testedComponent;
			...
			it( 'wrapped component has expected props', () => {
				expect( testedComponent.props.registration ).toBe(
					registration
				);
				expect( testedComponent.props.datetimeId ).toBe( datetimeId );
				expect( testedComponent.props.checkinEntity ).toEqual( {
					checkIn: true,
				} );
