...
describe( 'persistEntityRecord()', () => {
	...
	describe( 'yields expected actions when entity is new', () => {
		...
		it(
			'yields expected action object for ' + removeEntityById.name,
			() => {
				...
				expect( value ).toEqual(
					removeEntityById( 'event', NewEvent.id )
				);
			}
		);
		it(
...
describe( 'persistForEntityIds()', () => {
	...
	let fulfillment;
	const reset = () =>
		( fulfillment = persistForEntityIds( 'event', [ 20, 30 ] ) );
	it(
		...
		() => {
			...
			const { value } = fulfillment.next();
			expect( value ).toEqual(
				select( CORE_REDUCER_KEY, 'getEntitiesByIds', 'event', [
					20,
					30,
				] )
			);
...
describe( 'persistDeletesForModel', () => {
	...
	it(
		...
		() => {
			...
			expect( removeAction2 ).toEqual(
				removeDeleteEntityId( 'event', 10 )
			);
		}
	);
