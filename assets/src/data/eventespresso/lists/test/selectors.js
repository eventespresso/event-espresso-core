...
describe('testing getters', () => {
	const testConditions = [
		[
			Map().set(
				'generic',
				Map().set('some_query_string=1', Set.of({ id: 1 }))
			),
			getItems,
			'getItems()',
			'identifier',
		...
		[
			Map().set(
				'event',
				Map().set('some_query_string=1', OrderedMap().set(1, {}))
			),
			getEntities,
			'getEntities()',
			'modelName',
	...
	testConditions.forEach(
		([
			...
			describe(describeDescription, () => {
				...
				it(
					'returns expected default value when ' +
						testDescriptionPart +
...
describe('getEntitiesByIds()', () => {
	...
	it(
		"returns expected default value when model doesn't exist in the " +
			'state',
		() => {
			expect(getEntitiesByIds(originalState, 'cheese', [10, 20])).toEqual(
				[]
			);
		}
	);
	it(
		'returns expected default value when ids do not exist in the ' +
			'state (for the query being made)',
		() => {
			expect(getEntitiesByIds(originalState, 'event', [10])).toEqual([]);
		}
	);
	it('returns expected value for query matching state contents', () => {
		expect(getEntitiesByIds(originalState, 'event', [10, 20])).toEqual([
			{},
			{},
		]);
	});
	it('returns expected cached value on repeated calls', () => {
		const testResult = getEntitiesByIds(originalState, 'event', [10, 20]);
		expect(getEntitiesByIds(originalState, 'event', [10, 20])).toBe(
			testResult
		);
	});
});
...
describe('isRequesting()', () => {
	...
	function setIsResolving(isResolving, selectorKey = 'getItems') {
	...
	describe('isRequestingItems()', () => {
		...
		testConditions.forEach(
			([description, setResolving, expectedResponse]) => {
				it(description, () => {
