...
describe( 'getEntitiesByIds()', () => {
	...
	it(
		...
		() => {
			expect(
				getEntitiesByIds( originalState, 'cheese', [ 10, 20 ] )
			).toEqual( [] );
		}
	);
	it(
		...
		() => {
			expect(
				getEntitiesByIds( originalState, 'event', [ 10 ] )
			).toEqual( [] );
		}
	);
	it( 'returns expected value for query matching state contents', () => {
		expect(
			getEntitiesByIds( originalState, 'event', [ 10, 20 ] )
		).toEqual( [ {}, {} ] );
	} );
	it( 'returns expected cached value on repeated calls', () => {
		const testResult = getEntitiesByIds( originalState, 'event', [
			10,
			20,
		] );
