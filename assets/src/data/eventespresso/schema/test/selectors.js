...
describe( 'testing isRequesting and hasResolved methods', () => {
	...
	testConditions.forEach(
		( [ selectorMethod, methodDescription, extraArgs ] ) => {
			describe( selectorMethod.name + '()', () => {
				it(
					'returns false if never requested or finished ' +
						'requesting',
					() => {
						expect(
							selectorMethod(
				...
				it(
					'returns true if resolution is started and not ' +
						'finished',
					() => {
...
describe( 'hasJoinTableRelation()', () => {
	...
	it( 'returns true when there is a join table ' + 'relation', () => {
		expect(
			hasJoinTableRelation( mockStateForTests, 'event', 'venues' )
		).toBe( true );
	} );
	it(
		...
		() => {
			expect(
				hasJoinTableRelation( mockStateForTests, 'foo', 'bar' )
			).toBe( false );
		}
	);
} );
describe( 'getRelationType()', () => {
	it( 'returns expected value for relation that exists', () => {
		expect(
			getRelationType( mockStateForTests, 'event', 'datetimes' )
...
describe( 'getRelationSchema()', () => {
	...
	it( 'returns null for relation that does not exist', () => {
		expect(
			getRelationSchema( mockStateForTests, 'foo', 'bar' )
		).toBeNull();
	} );
} );
