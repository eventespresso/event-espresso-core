...
export class EventAttendeesEditor extends Component {
	...
	setEventId = (eventId) => {
		const value =
			eventId !== null && eventId.value ? parseInt(eventId.value, 10) : 0;
	...
	setTicketId = (ticketId) => {
		const value =
			ticketId !== null && ticketId.value ? parseInt(ticketId.value, 10) : 0;
...
export default withSelect((select, ownProps) => {
	...
	const { getAttendees, isRequestingAttendees } = select('eventespresso/lists');
	return {
