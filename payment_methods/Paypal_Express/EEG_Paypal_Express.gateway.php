...
class EEG_Paypal_Express extends EE_Offsite_Gateway
{
    ...
    public function set_redirection_info(
        ...
        $token_request_details = apply_filters(
            ...
            $this->token_request->generateDetails(
                ...
                isset($return_url) ? $return_url : '',
                isset($cancel_url) ? $cancel_url : '',
                isset($this->_image_url) ? $this->_image_url : '',
                isset($this->_request_shipping_addr) ? $this->_request_shipping_addr : ''
            ),
            $this
        );
     ...
     * @return mixed[]
     */
    private function getOrderItems(EE_Payment $payment, array $request_response_args = [])
    {
     ...
     * @return void
     */
    private function _log_clean_request(array $request, EE_Payment $payment, $info)
    {
        $info = (string) $info;
     ...
     * @return array
     */
    private function _get_errors(array $data_array)
    {
        $errors = [];
        $n      = 0;
    ...
    private function paymentApproved(EE_Payment $payment, array $checkout_response, array $customer_data)
    {
        ...
        $payment_amount = isset($checkout_response['PAYMENTINFO_0_AMT']) ? $checkout_response['PAYMENTINFO_0_AMT'] : 0;
        $payment->set_amount((float) $payment_amount);
        $payment->set_txn_id_chq_nmbr(isset($checkout_response['PAYMENTINFO_0_TRANSACTIONID']) ? $checkout_response['PAYMENTINFO_0_TRANSACTIONID'] : null);
        $payment->set_details($customer_data);
        $payment->set_gateway_response(isset($checkout_response['PAYMENTINFO_0_ACK']) ? $checkout_response['PAYMENTINFO_0_ACK'] : '');
     ...
     * @since   $VID:$
     */
    private function processPayment(EE_Payment $payment, $payment_token, array $customer_details)
    {
     ...
     * @since   $VID:$
     */
    private function requestCustomerDetails(EE_Payment $payment, $payment_token)
    {
        $payment_token = (string) $payment_token;
     ...
     * @since   $VID:$
     */
    private function validateTransaction(EE_Payment $payment, $transaction = null)
    {
        $transaction = isset($transaction) ? $transaction : $payment->transaction();
