...
class EED_PayPalOnboard extends EED_Module
{
     ...
     * @throws Exception
     */
    public static function getSignUpLink($paypal_pm)
    {
     ...
     * @throws Exception
     */
    public static function requestOnboardingUrl($paypal_pm)
    {
        $signup_link = '';
     ...
     * @throws Exception
     */
    public static function getPartnerAccessToken($paypal_pm)
    {
        // Do we have it saved ?
     ...
     * @throws Exception
     */
    public static function partnerAccessTokenExpired($paypal_pm)
    {
     ...
     * @throws EE_Error
     */
    public static function requestPartnerAccessToken($paypal_pm)
    {
     ...
     * @throws Exception
     */
    public static function requestApiCredentials($paypal_pm, $seller_token)
    {
        ...
        if (empty($response['client_id']) || empty($response['client_secret'])) {
            ...
            if (isset($response['message'])) {
                ...
                $err_name = isset($response['name']) ? $response['name'] : 'UNRECOGNIZED_ERROR';
            } else {
                $err_msg  = esc_html__('Incoming parameter validation failed.', 'event_espresso');
    ...
    public static function getOnboardStatus()
    {
        ...
        try {
            $seller_id = PayPalExtraMetaManager::getPmOption($paypal_pm, Domain::META_KEY_PAYER_ID) !== null ? PayPalExtraMetaManager::getPmOption($paypal_pm, Domain::META_KEY_PAYER_ID) : '--';
        } catch (Exception $e) {
            $seller_id = '--';
        }
        echo json_encode(
     ...
     * @throws EE_Error
     */
    public static function sendRequest($paypal_pm, $request_url, $request_args)
    {
        ...
        if (empty($response_body) || isset($response_body['error'])) {
            $message = isset($response_body['error_description']) ? $response_body['error_description'] : sprintf(
     ...
     * @throws EE_Error
     */
    public static function partnerTokenResponseValid($response, $paypal_pm)
    {
        // Check the data we received.
        if (
     ...
     * @return string
     */
    public static function getMiddlemanBaseUrl($payment_method)
    {
     ...
     * @return string
     */
    public static function getPayPalApiUrl($payment_method)
    {
     ...
     * @throws EE_Error
     */
    public static function isOnboard($payment_method)
    {
     ...
     * @return void
     */
    public static function exitWithError($error_message = '', $show_alert = false)
    {
        echo json_encode(
            [
                'error'   => $error_message,
