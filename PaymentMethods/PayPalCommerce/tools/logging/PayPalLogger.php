...
class PayPalLogger
{
     ...
     * @return array
     */
    private static function cleanDataArray(array $data)
    {
        $sensitive_data = [
            'access_token',
            'refresh_token',
        ...
        foreach ($data as $key => $value) {
            if (is_string($value)) {
                // Json encoded ?
                $value = json_decode($value) !== null ? json_decode($value) : $value;
            }
     ...
     * @return void
     */
    public static function logInWindow($message)
    {
     ...
     * @return void
     */
    public static function closeWindowAndExit($message = '')
    {
        $js_out = '<script type="text/javascript">';
        if (! empty($message)) {
            $js_out .= '
