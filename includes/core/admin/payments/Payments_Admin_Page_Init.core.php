...
class Payments_Admin_Page_Init extends EE_Admin_Page_Init {
	...
	public function __construct() {
		...
		add_action('wp_loaded',array($this,'check_payment_gateway_setup'));
		parent::__construct();
	}
	...
	/**
	 * Checks that there is at least one active gateway. If not, adds an admin notice to the user
	 */
	public function check_payment_gateway_setup(){
		$actives = EEM_Gateways::instance()->active_gateways();
		if( ! $actives || count($actives) < 1){
			add_action('admin_notices',array($this,'show_no_payment_gateway_selected_notice'));
		}
	}
	/**
	 * Displays the admin warning stating that there are no active gateways
	 */
	public function show_no_payment_gateway_selected_notice(){
