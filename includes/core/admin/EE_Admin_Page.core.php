...
abstract class EE_Admin_Page extends EE_BASE {
	 ...
	 *     		'help_tour' => array(
	 *     			'label' => __('Some Tour', 'event_espresso'), //a label for this tour that is used for the autogenerated restart tour button (required)
	 *     			'id' => 'id_of_joyride_skeleton', //this is a unique id for the joyride skeleton containing the content for the stops.  Just provide an id, The generator will take care of the rest. (required)
	 *				'stop_callback' => '_stop_callback', //this is the method that gets called to return our stops setup array for the tour. (see the sample callback in this class) (required)
	 *				'options_callback' => '_help_tour_options_callback' //this is the method that gets called to return the global options setup for the tour. (optional).  If not included then default options are used.  (see the sample callback in this class for the format of the array returned) (optional)
	 *     		)
		...
		if ( isset( $this->_page_config[$this->_req_action] ) ) {
			...
			if ( isset( $this->_help_tour[$this->_req_action]) ) {
				...
				foreach ( $this->_help_tour['tours'] as $tour ) {
					$tour_buttons .= '<button id="trigger-tour-' . $tour['id'] . '" class="button-primary trigger-ee-help-tour">' . $tour['label'] . '</button>';
				}
	...
	protected function _add_help_tour() {
		...
		foreach ( $this->_page_config as $route => $config ) {
			...
			if ( isset( $config['help_tour'] ) ) {
				...
				if ( !isset( $config['help_tour']['label'] ) )
					throw new EE_Error( sprintf( __('A helper tour is defined in the $_page_config for the %s route, but is missing a defined label.  Double check and make sure it is added to the help_tour configuration array', 'event_espresso'), $route ) );


				//let's setup our vars to send to the tour builder
				$id = $config['help_tour']['id'];
				$label = $config['help_tour']['label'];
