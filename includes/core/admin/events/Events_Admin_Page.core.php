		...
		<div id="ticket-prices-dv" class="">
			...
			<?php if ($show_no_event_price_msg) : ?>
				...
				<div id="no-ticket-prices-msg-dv">
					<p>
						<?php echo apply_filters('espresso_hook_espresso_show_no_event_price_msg', __('Please enter at lease one Event Price for this Event to ensure that this Event displays and functions properly.', 'event_espresso')); ?>
					</p>
				</div>
			<?php endif; ?>
									...
									<table class="ticket-price-quick-edit-tbl" width="100%">
										<tr>
											...
											<td class="type-column ticket-price-quick-edit-column"> 
												...
												<span><?php echo $price->type_obj()->name() . ' ' . $price_date_status; ?></span>
											</td> 
											...
											<td class="amount-column ticket-price-quick-edit-column"> 
												...
												<?php $price_amount = $price->type_obj()->is_percent() ? number_format($price->amount(), 1) : number_format($price->amount(), 2); ?>
...
//		}
	...
	private function _insert_event() {
		...
		foreach ($question_groups as $question_group) {
			$wpdb->insert(
					$wpdb->prefix . 'esp_event_question_group', array('EVT_ID' => $last_event_id, 'QSG_ID' => $question_group, 'EQG_primary' => 1), array('%d', '%d', '%d')
			);
		}
		...
		foreach ($add_attendee_question_groups as $question_group) {
			$wpdb->insert(
					$wpdb->prefix . 'esp_event_question_group', array('EVT_ID' => $last_event_id, 'QSG_ID' => $question_group, 'EQG_primary' => 0), array('%d', '%d', '%d')
			);
		}
...
//                    echo printr( $new_price, '$new_price' );
		...
		if (empty($error)) {
			...
			$msg = sprintf(
					__('The event %s has been added for %s.', 'event_espresso'), '<a href=' . espresso_reg_url($last_event_id) . '>' . stripslashes_deep($this->_req_data['event']) . '</a>', $evt_date
			);
			EE_Error::add_success($msg);
		} else {

			$msg = sprintf(
					__('An error occured and the event %s has not been saved to the database.', 'event_espresso'), '<a href=' . espresso_reg_url($last_event_id) . '>' . stripslashes_deep($this->_req_data['event']) . '</a>'
			);
			EE_Error::add_error($msg, __FILE__, __FUNCTION__, __LINE__);
		}
	...
	private function _update_event() {
		...
		foreach ($question_groups as $question_group) {
			$wpdb->insert(
					$wpdb->prefix . 'esp_event_question_group', array('EVT_ID' => $event_id, 'QSG_ID' => $question_group, 'EQG_primary' => 1), array('%d', '%d', '%d')
			);
		}
		...
		foreach ($add_attendee_question_groups as $question_group) {
			$wpdb->insert(
					$wpdb->prefix . 'esp_event_question_group', array('EVT_ID' => $event_id, 'QSG_ID' => $question_group, 'EQG_primary' => 0), array('%d', '%d', '%d')
			);
		}
...
//die();
		...
		if (empty($error)) {
			...
			$msg = sprintf(
					__('The event %s has been updated', 'event_espresso'), '<a href=' . espresso_reg_url($event_id) . '>' . stripslashes_deep($this->_req_data['event']) . '</a>'
			);
			EE_Error::add_success($msg);
		} else {

			$msg = sprintf(
					__('An error occured and the event %s has not been updated in the database.', 'event_espresso'), '<a href=' . espresso_reg_url($event_id) . '>' . stripslashes_deep($this->_req_data['event']) . '</a>'
			);
			EE_Error::add_error($msg, __FILE__, __FUNCTION__, __LINE__);
		}
