...
class EE_Attendees_List_Table extends EE_Admin_List_Table {
	...
	protected function _setup_data() {
		...
		$this->_data = $this->_view != 'trash' ? $this->_admin_page->get_attendees() : $this->_admin_page->get_attendees(FALSE,TRUE);
	}
	...
	protected function _add_view_counts() {
		$this->_views['in_use']['count'] = $this->_admin_page->get_attendees(TRUE);
		$this->_views['trash']['count'] = $this->_admin_page->get_attendees(TRUE, TRUE);
	}
	...
	function column_ATT_fname($item) {
		...
		if ( $this->_view == 'in_use' ) {
			// trash attendee link
			$trash_lnk_url = wp_nonce_url( add_query_arg( array( 'action'=>'trash_attendees', 'id'=>$item->ID() ), ATT_ADMIN_URL ), 'trash_attendees_nonce' );
			...
			$restore_lnk_url = wp_nonce_url( add_query_arg( array( 'action'=>'restore_attendees', 'id'=>$item->ID() ), ATT_ADMIN_URL ), 'restore_attendees_nonce' );
			...
			$delete_lnk_url = wp_nonce_url( add_query_arg( array( 'action'=>'delete_attendees', 'id'=>$item->ID() ), ATT_ADMIN_URL ), 'delete_attendees_nonce' );
