...
?>
          <div class="metabox-holder">
            ...
            <div class="padding">
              ...
              <li><label for="default_payment_status">
			  <?php
 ...
 _e(' Default Payment Status (this value will be automatically filled in for each person\'s payment status, until payment is made, for each event): ','event_espresso'); ?></label>
              <?php echo select_input('default_payment_status', $default_payment_status, $org_options['default_payment_status'])?>
               <?php /*?> <label for="default_payment_status">
                    
              <?php _e(' Default Payment Status (this value will be automatically filled in for each person\'s payment status, until payment is made, for each event): ','event_espresso'); ?>
                </label>
                <select name="default_payment_status">
                        <option value = "Incomplete" <?php selected($org_options['default_payment_status'], 'Incomplete') ?>>Incomplete</option>
                        <option value = "Pending" <?php selected($org_options['default_payment_status'], 'Pending') ?>>Pending</option>
                        <option value = "Completed" <?php selected($org_options['default_payment_status'], 'Completed') ?>>Completed</option>
                    </select><?php */?>
              </li>
              ...
              <li>
                  ...
                  <?php	echo select_input('espresso_dashboard_widget', $values, isset($org_options['espresso_dashboard_widget']) ? $org_options['espresso_dashboard_widget'] : '');?>
                	<?php _e('Show the next', 'event_espresso'); ?> <input name="events_in_dasboard" size="5" style="width:50px;" type="text" value="<?php echo isset($org_options['events_in_dasboard']) || $org_options['events_in_dasboard'] == ''? '30':stripslashes_deep($org_options['events_in_dasboard']);?>" /> <?php _e('days of events in the dashboard.', 'event_espresso'); ?>
                </li>
                <li>
                  <label for="time_reg_limit">
                    <?php _e('Use registration limits on time slots?<br />
							<em class="important">(This function is experimental and may not function as expected. You should adjust your attendee limit accordingly.)</em>','event_espresso'); ?></label>
                ...
                <?php if (get_option('event_espresso_multi_reg_active') == 1){?>
                    <li>
                      <label for="display_description_on_multi_reg_page">
                        <?php _e('Display event descriptions in the multiple event registration pages?','event_espresso'); ?></label> 
                ...
                <li>
                  <label>
                    <?php _e('Show a link to Event Espresso in your event pages?','event_espresso'); ?></label>
                  <?php echo select_input('show_reg_footer', $values, isset($org_options['show_reg_footer'])?$org_options['show_reg_footer']:''); ?>
                     <?php _e('Affiliate ID:', 'event_espresso'); ?> <input name="affiliate_id" size="10" style="width:70px;" type="text" value="<?php echo isset($org_options['affiliate_id'])&&$org_options['affiliate_id'] != ''? stripslashes_deep($org_options['affiliate_id']):'0';?>" /> <?php _e('(optional)', 'event_espresso'); ?>
