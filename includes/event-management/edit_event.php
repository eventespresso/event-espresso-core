...
function edit_event($event_id = 0) {
    ...
    <?php _e('Start Date', 'event_espresso'); ?>
                                        <b> <?php echo event_date_display($start_date); ?> <?php echo event_date_display($start_time, get_option('time_format')); ?></b>
                                    </span>
                                </div>

                                <div class="misc-pub-section">
                                    <label for="post_status"><?php _e('Current Status:', 'event_espresso'); ?></label>
                                    <span id="post-status-display"> <?php echo $status['display']; ?></span>
                                </div>
    ...
    <?php else: ?>
                                <div id="delete-action">
                                    <a class="submitdelete deletion" href="admin.php?page=events&amp;action=delete&event_id=<?php echo $event_id ?>" onclick="return confirm('<?php _e('Are you sure you want to delete ' . $event_name . '?', 'event_espresso'); ?>')">
                        ...
                        <input type="checkbox" name="espresso_fb" id="espresso_fb" <?php echo ($fb_e_id ? "CHECKED=TRUE" : null); ?>/> <?php _e('Post to Facebook', 'event_espresso'); ?>
                           <?php if (!empty($fb_e_id)) { ?>
                            <a href="http://www.facebook.com/event.php?eid=<?php echo $fb_e_id; ?>"
                               target="_blank"><?php _e('Event Page On Facebook', 'event_espresso'); ?></a>
                <?php } ?>
                    </div>
                </div>
    <?php } ?>
    ...
    <?php _e('Event Category', 'event_espresso'); ?>
                ...
                <div class="inside">
    <?php echo event_espresso_get_categories($event_id); ?>
                </div>
            </div>
            <!-- /event-category -->
        ...
        <?php _e('Event Staff / Speakers', 'event_espresso'); ?>
                    ...
                    <div class="inside">
                <?php echo espresso_personnel_cb($event_id); ?>
                    </div>
                </div>
            <?php
            }
    ...
    <?php echo '<a href="#" class="button" onclick="prompt(&#39;Event TinyURL:&#39;, \'' . espresso_getTinyUrl(espresso_reg_url($event_id)) . '\'); return false;">' . __('TinyURL') . '</a>' ?>
    ...
    <?php _e('Event Date/Times', 'event_espresso'); ?>
                    ...
                    <div class="inside">
                        ...
                        <table width="100%" border="0" cellpadding="5">
                            <tr valign="top">
                                <td class="a">
                                    <fieldset id="add-reg-dates">
                                        <legend><?php _e('Registration Dates', 'event_espresso'); ?></legend>
                                        <p><label for="registration_start"> <?php echo __('Registration Start:', 'event_espresso') ?></label><input type="text" class="datepicker" size="15" id="registration_start" name="registration_start"  value="<?php echo $registration_start ?>" /></p>
                                        <p><label for="registration_end"><?php echo __('Registration End:', 'event_espresso') ?></label> <input type="text" class="datepicker" size="15" id="registration_end" name="registration_end"  value="<?php echo $registration_end ?>" /></p>
                                    </fieldset>

                                    <fieldset>
                                        <legend><?php _e('Event Dates', 'event_espresso'); ?></legend>
                                        <p><label for="start_date"><?php echo __('Event Start Date', 'event_espresso') ?></label> <input type="text" class="datepicker" size="15" id="start_date" name="start_date" value="<?php echo $start_date ?>" /></p>
                                        <p><label for="end_date"><?php echo __('Event End Date', 'event_espresso') ?></label> <input type="text" class="datepicker" size="15" id="end_date" name="end_date" value="<?php echo $end_date ?>" /></p>
                                    </fieldset>
                                    ...
                                    <?php if (isset($org_options['use_event_timezones']) && $org_options['use_event_timezones'] == 'Y' && $espresso_premium == true) { ?>
                                        <fieldset id="event-timezone">
                                            <p><label><?php _e('Event Timezone', 'event_espresso') ?>:</label><?php echo eventespresso_ddtimezone($event_id) ?></p>
                                        </fieldset>
                                <?php } ?>
                                </td>
    ...
    <?php // ADD TIME REGISTRATION  ?>
                                ...
                                <td class="b">
                                    <fieldset id="add-register-times">
                                        <legend><?php _e('Registration Times', 'event_espresso'); ?></legend>
    ...
    <?php echo event_espresso_timereg_editor($event_id); ?>
                                    ...
                                    <fieldset id="add-event-times">
                                        <legend><?php _e('Event Times', 'event_espresso'); ?></legend>
                                    ...
                                    <?php if ((!isset($org_options['use_event_timezones']) || $org_options['use_event_timezones'] != 'Y') && $espresso_premium == true) { ?>
                                        <p><span class="run-in"><?php _e('Current Time', 'event_espresso'); ?>:</span>
                                            <span class="current-date"> <?php echo date(get_option('date_format')) . ' ' . date(get_option('time_format')); ?></span> <a class="change-date-time" href="options-general.php" target="_blank"><?php _e('Change timezone and date format settings?', 'event_espresso'); ?></a></p>
                                    ...
                                    <td <?php echo $ven_type ?>>
                                        <fieldset id="venue-manager">
                                            ...
                                            <?php if (!espresso_venue_dd()) : ?>
                                                <p class="info"><b><?php _e('You have not created any venues yet.', 'event_espresso'); ?></b></p>
        ...
        ?>
                                    <td <?php echo $ven_type ?>>
                                        <fieldset>
                                            ...
                                            <p>
                                                <label for="phys-addr"> <?php _e('Address:', 'event_espresso'); ?></label>
    ...
    <?php } ?>
                                ...
                                <td <?php echo $ven_type ?>>
                                    <fieldset id="virt-location">
                                        <legend><?php _e('Virtual Location', 'event_espresso'); ?></legend>
                                        <p>
                                            <label for="virt-phone"><?php _e('Phone:', 'event_espresso'); ?></label>

                                            <input size="20" id="virt-phone" type="text" tabindex="111" value="<?php echo $phone ?>" name="phone" />
                                        </p>
                                        <p>
                                            <label for="url-event"><?php _e('URL of Event:', 'event_espresso'); ?></label>

                                            <textarea id="url-event" cols="30" rows="4" tabindex="112"  name="virtual_url"><?php echo $virtual_url ?></textarea>
                                        </p>
                                        <p>
                                            <label for="call-in-num"><?php _e('Call in Number:', 'event_espresso'); ?></label>
