...
function espresso_total_events_this_month(){	
	...
	function espresso_total_all_attendees(){
		...
		if (function_exists('espresso_member_data')&&espresso_member_data('role')=='espresso_group_admin'){
			...
			$asql1 .= " AND quantity >= 1 AND (payment_status='Completed' OR payment_status='Pending') ";
			$asql1 .= " AND event_status != 'D' ";
			$asql1 .= ") UNION (";
		}
		$asql1 .= "SELECT SUM(a.quantity) quantity FROM " . EVENTS_ATTENDEE_TABLE. " a ";
		$asql1 .= " JOIN ". EVENTS_DETAIL_TABLE ." e ON e.id=a.event_id ";
		...
		$asql1 .= " AND a.quantity >= 1 AND (a.payment_status='Completed' OR a.payment_status='Pending') ";
		$asql1 .= " AND e.event_status != 'D' ";
		$asql1 .= ")";
		if(  function_exists('espresso_member_data') && ( espresso_member_data('role')=='espresso_event_manager' || espresso_member_data('role')=='espresso_group_admin') ){
		}
		$wpdb->query($asql1);
	...
	function espresso_total_attendees_today(){
		...
		if (function_exists('espresso_member_data')&&espresso_member_data('role')=='espresso_group_admin'){
			...
			$asql2 .= " AND quantity >= 1 AND (payment_status='Completed' OR payment_status='Pending') ";
		...
		$asql2 .= " AND a.quantity >= 1 AND (a.payment_status='Completed' OR a.payment_status='Pending') ";
	...
	function espresso_total_attendees_this_month(){
		...
		if (function_exists('espresso_member_data')&&espresso_member_data('role')=='espresso_group_admin'){
			...
			$asql3 .= " AND a.quantity >= 1 AND (a.payment_status='Completed' OR a.payment_status='Pending') ";
