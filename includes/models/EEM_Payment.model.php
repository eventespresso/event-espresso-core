...
class EEM_Payment extends EEM_Base {
	...
	*/	
	public function get_payments_for_transaction( $TXN_ID = FALSE ) {
		...
		if ( ! $TXN_ID ) {
			...
			$espresso_notices['errors'][] = __('No Transaction ID was supplied.', 'event_espresso') . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
	...
	*/
	public function update_payment_transaction( EE_Payment $payment, $what ) {
		...
		if ( ! is_object( $payment ) && ! $payment->ID() ) {
			$espresso_notices['errors'][] = __('A vaild payment object was not supplied.', 'event_espresso') . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
		...
		if ( $transaction->update() ) {
			...
			$espresso_notices['errors'][] = __('An error occured. The payment was ' . $what . ' succesfully but the amount paid for the transaction was not updated.', 'event_espresso') . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
	...
	*/
	public function recalculate_total_payments_for_transaction( $TXN_ID = FALSE ) {
		...
		if ( ! $TXN_ID ) {
			global $espresso_notices;
			$espresso_notices['errors'][] = 'No Transaction ID was supplied.' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
	...
	*/
	public function delete_payment( $PAY_ID ) {
		...
		global $espresso_notices;
		if ( ! $PAY_ID ) {
			$espresso_notices['errors'][] = __('No Payment ID was supplied.', 'event_espresso') . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
		...
		if( $payment = $this->get_payment_by_ID( $PAY_ID )) {
			...
			if ( $this->delete ( array( 'PAY_ID' => $payment->ID() ))) {
				...
				$espresso_notices['errors'][] = __('An error occured. The payment has not been deleted succesfully.', 'event_espresso') . 
																	EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
				return FALSE;
			}
