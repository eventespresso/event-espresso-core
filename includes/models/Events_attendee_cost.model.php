 ...
 * ------------------------------------------------------------------------
 *
 * Events Attendee Cost Model
 * 
 *
 * @package				Event Espresso
 * @subpackage		mvc-models
 * @author					Brent Christensen 
 * @table 						wp_events_attendee_cost
 *
 * ------------------------------------------------------------------------
 */
class Events_attendee_cost extends espresso_model {

	protected $table_name = EVENTS_ATTENDEE_COST_TABLE;
...
/**
 *		This function calculates the total cost for all event attendees
 *
 * example: SELECT ea.registration_id, SUM(eac.cost * eac.quantity) FROM wp_events_attendee_cost eac INNER JOIN wp_events_attendee ea ON eac.attendee_id = ea.id WHERE ea.registration_id = '4e7962a15f3b72.67409710' GROUP BY ea.registration_id
 *
 *		Used within the following files: edit_attendee_record.php
 *		
 *		@access public
 *		@param mixed - int or array of registration IDs (ints)
 *		@return array
 */	
	public function Calculate_total_attendee_cost( $reg_IDs = FALSE ) {
	
		// what?? no registration IDs ??? Get outta here!!!
		if ( ! $reg_IDs ) {
			return array( 'type' =>  'error', 'msg' => 'The cost could not be calculated because no Registration ID was supplied.', 'rows' => FALSE );
		}
		
		// got something but it's not an array?
		if ( ! is_array( $reg_IDs )) {
		...
		foreach ( $reg_IDs as $reg_ID ) {
			$SQL = "SELECT ea.registration_id, SUM(eac.cost * eac.quantity) FROM ".$this->table_name." eac INNER JOIN ".EVENTS_ATTENDEE_TABLE." ea ON eac.attendee_id = ea.id WHERE ea.registration_id = '%s' GROUP BY ea.registration_id";
			$event_cost += $wpdb->get_var($wpdb->prepare( $SQL, $reg_ID ),1,0);
		}
...
/**
 *		This function deletes all records from the attendee cost table
 *		where no corresponding id exists in the attendee table
 *
 ...
 */	
	public function cleanup_attendee_cost_data() {
	
		global $wpdb;
		// delete garbage records
		$SQL = 'DELETE eac FROM ' . $this->table_name . ' eac LEFT JOIN ' . EVENTS_ATTENDEE_TABLE . ' ea ON eac.attendee_id = ea.id WHERE ea.id IS NULL';
 ...
 */	
	public function insert ($set_column_values) {
		...
		if ( $results['rows'] == 1 ) {
			...
			$results['msg'] = 'Attendee cost details have been successfully saved to the database.';
		} elseif ( $results['rows'] > 1 ) {
			// multiple rows were successfully updated
			$results['msg'] = 'Cost details for '.$results.' attendees have been successfully saved to the database.';
		} else {
			// error message 
			$results['msg'] = 'An error occured and the attendee cost details have not been saved to the database.';
		}
 ...
 */	
	public function update ($set_column_values, $where) {
		...
		if ( $results['rows'] == 1 ) {
			// one row was successfully updated
			$results['msg'] = 'Attendee cost details have been successfully updated.';
		} elseif ( $results['rows'] > 1 ) {
			// multiple rows were successfully updated
			$results['msg'] = 'Cost details for '.$results.' attendees have been successfully updated.';
		} else {
			// error message 
			$results['msg'] = 'An error occured and the attendee cost details have not been updated.';
		}
 ...
 *		@ return string
 */	
	public function get_error_code (  $file, $func, $line ) {
		$results = parent::eedb_get_error_code (  $file, $func, $line );
		return $results;
	}
...
// End of file Events_attendee_cost.model.php
// Location: /ee-mvc/models/Events_attendee_cost.model.php