...
class EE_WP_Post_Status_Field extends EE_Enum_Field{
	 ...
	 * @param array  $new_stati           If additional stati are to be used other than the default WP statuses then they can be registered via this property.  The format of the array should be as follows:
	 * array(
	 * 'status_reference' => array(
	 * 		'label' => __('Status Reference Label', 'event_espresso')
	 * 		'public' => true, //'Whether posts of this status should be shown on the frontend of the site'
	 * 		'exclude_from_search' => false, //'Whether posts of this status should be excluded from wp searches'
	 * 		'show_in_admin_all_list' => true, //whether posts of this status are included in queries for the admin "all" view in list table views.
	 * 		'show_in_admin_status_list' => true, //Show in the list of statuses with post counts at the top of the admin list tables (i.e. Status Reference(2) )
	 * 		'label_count' => _n_noop( 'Status Reference <span class="count">(%s)</span>', 'Status References <span class="count">(%s)</span>' ), //the text to display on the admin screen( or you won't see your status count ).
	 *   )
	 * )
	 ...
	 */
	function __construct($table_column, $nicename, $nullable, $default_value, $new_stati, $store_in_db_as_int = false){
