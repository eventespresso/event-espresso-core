...
class Event_datetime extends Espresso_model {
	...
	private function __construct() {	
		...
		// set table name
		$this->table_name = $wpdb->prefix . 'esp_event_datetime';
	...
	*/	
	private function _get_event_datetimes( $EVT_ID = FALSE, $DTT_event_or_reg = FALSE, $start_or_end = FALSE ) {
		...
		$where = array( 'EVT_ID' );
		$where_value = array( $EVT_ID );			

		if ( $DTT_event_or_reg ) {
			$where[] = 'DTT_event_or_reg';
			$where_value[] = $DTT_event_or_reg;
		} 
		
		if ( $start_or_end ) {
			$where[] = 'DTT_start_or_end';
			$where_value[] = $start_or_end;
		} 
		
		$orderby = 'DTT_timestamp';
//	echo printr( $where, '$where' );
//	echo printr( $where_value, '$where_value' );
		
		if ( $datetimes = $this->EEDB->_select_all_where ( $this->table_name, $this->table_data_types, $where, $where_value, $orderby )) {
...
//		echo printr( $events, '$events' );
	...
	public function select_all_where ( $where=FALSE, $where_value=FALSE, $orderby = FALSE, $sort = 'ASC', $operator = '=', $output = 'OBJECT_K' ) {
		$results = $this->EEDB->_select_all_where ( $this->table_name, $this->table_data_types, $where, $where_value, $orderby, $sort, $operator, $output );
		return $results;
	}
	...
	public function select_row_where ( $where=FALSE, $where_value=FALSE, $operator = '=', $output = 'OBJECT_K' ) {
		$results = $this->EEDB->_select_row_where ( $this->table_name, $this->table_data_types, $where, $where_value, $operator, $output );
		return $results;
	}
	...
	public function select_value_where ( $select=FALSE, $where=FALSE, $where_value=FALSE, $operator = '=' ) {
		$results = $this->EEDB->_select_value_where ( $this->table_name, $this->table_data_types, $select, $where, $where_value, $operator );
		return $results;
	}
	...
	public function get_key_value_array_where( $key=FALSE, $value=FALSE, $where=FALSE, $where_value=FALSE, $orderby=FALSE, $sort='ASC', $operator='=' ) {
		$results = $this->EEDB->_get_key_value_array_where ( $this->table_name, $this->table_data_types, $key, $value, $where, $where_value, $orderby, $sort, $operator );
		return $results;
	}
	...
	protected function eedb_delete ( $where=FALSE, $where_value=FALSE, $operator = '=' ) {
		// grab data types from above and pass everything to espresso_model (parent model) to perform the update
		$results = $this->EEDB->_update( $this->table_name, $this->table_data_types, $where, $where_value, $operator );
		return $results;
	}
...
// End of file Event_datetime.model.php
// Location: /includes/models/Event_datetime.model.php