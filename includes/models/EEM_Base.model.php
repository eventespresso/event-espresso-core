...
abstract class EEM_Base extends EE_Base {
	...
	private function _select_all (  $em_table_name=FALSE, $orderby = FALSE, $sort = 'ASC', $output = 'OBJECT_K' )	{
		...
		if ( ! $em_table_name ) {
			$espresso_notices['errors'][] = 'No table has been specified. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
	...
	private function _select_value_where ( $em_table_name=FALSE, $em_table_data_types=array(), $select=FALSE, $where_cols_n_values=FALSE, $operator = '=' ) {
		...
		if ( ! $select ) {
			$espresso_notices['errors'][] = 'No column name has been specified for the SELECT clause. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
	...
	private function _get_key_value_array ( $em_table_name=FALSE, $em_table_data_types=array(), $key=FALSE, $value=FALSE, $orderby = FALSE, $sort = 'ASC', $output = 'OBJECT_K' ) {
		...
		// ya gotta let us know what you want!!!
		if ( ! $key or ! $value ) {
			$espresso_notices['errors'][] = 'Column names for both the "key" and "value" need to be provided. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
	...
	protected function _insert( $em_table_name=FALSE, $em_table_data_types=array(), $set_cols_n_values=array() ) {
		...
		if ( ! $em_table_name or empty($em_table_data_types) or empty($set_cols_n_values) ) {
			$espresso_notices['errors'][] = 'The insert can not be performed because of missing data. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
		...
		if ( ! is_array($em_table_data_types) or ! is_array($set_cols_n_values) ) {
			$espresso_notices['errors'][] = 'The insert can not be performed because the supplied data is of the wrong type. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
		...
		foreach ( $set_cols_n_values as $column => $value ) {
			...
			if ( array_key_exists( $column, $em_table_data_types )) {
				...
				$espresso_notices['errors'][] = 'The column name '. $column .' does not exist. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
				return FALSE;
			}
		}
...
//			$update_results = array( 'type' =>  'updated', 'msg' => $results.' records have been successfully created.', 'rows' => $row_results );
			...
			$espresso_notices['errors'][] = 'An error occured and the ' . $this->singlular_item . ' has not been saved to the database. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
//			$update_results = array( 'type' =>  'error', 'msg' => 'An error occured and the record was not created.', 'rows' => 0 );
	...
	protected function _update( $em_table_name=FALSE, $em_table_data_types=array(), $set_cols_n_values=array(), $where_cols_n_values=array() ) {
		...
		if ( ! $em_table_name or empty($em_table_data_types) or empty($set_cols_n_values) or empty($where_cols_n_values) ) {
			$espresso_notices['errors'][] = 'The update can not be performed because of missing data. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}

		// if any of the supplied data arrays are not arrays - send them back with an error
		if ( ! is_array($em_table_data_types) or ! is_array($set_cols_n_values) or ! is_array($where_cols_n_values) ) {
			$espresso_notices['errors'][] = 'The update can not be performed because the supplied data is of the wrong type. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
		...
		foreach ( $where_cols_n_values as $column => $value ) {
			...
			if ( array_key_exists( $column, $em_table_data_types )) {
				...
				$espresso_notices['errors'][] = 'The column name '. $column .' used in the WHERE clause, does not exist. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
				return FALSE;
			}
		}
...
//			$update_results = array( 'type' =>  'error', 'msg' => "The following error occured and the record was not updated : \n" . $error, 'rows' => FALSE );
	...
	private function _delete ( $em_table_name=FALSE, $em_table_data_types=array(), $where_cols_n_values=FALSE, $operator = '=' ) {
		...
		if ( $where_cols_n_values ) {
			...
			$espresso_notices['errors'][] = 'At least one column name and value has to be specified in order to delete data. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
...
//		echo printr( $em_table_data_types, '$em_table_data_types' );
		...
		if ( ! $where_cols_n_values ) {
			$espresso_notices['errors'][] = 'No coulmn name was provided for the WHERE clause. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}

		// what??? no table data types??? get outta here!!
		if ( ! $em_table_data_types ) {
			$espresso_notices['errors'][] = 'The table data types array is missing. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
	...
	private function _array_to_csv ($array=array()) {
		...
		if ( empty($array)) {
			$espresso_notices['errors'][] = 'The supplied array was empty. ' . EE_Error::generate_error_code (  __FILE__, __FUNCTION__, __LINE__ );
			return FALSE;
		}
