 ...
 * @table 						wp_events_detail
 *
 * ------------------------------------------------------------------------
 */
class Events_detail extends espresso_model {

	protected $table_name = EVENTS_DETAIL_TABLE;
	
	// array representation of wp_events_start_end table and the data types for each field
	private $table_data_types = array(
				'id' => '%d', 
				'event_code' => '%s',
				'event_name' => '%s',
				'event_desc' => '%s',
...
/**
 *		This function returns serialized questions and meta data from the wp_events_details table
 *
 *		exampe: 
 *		
 *		@access public
 ...
 *		@return array
 */	
	public function get_questions_and_event_meta ( $event_id = FALSE ) {
	
		// what?? no start_time_id ??? Get outta here!!!
		if ( ! $event_id ) {
			return array( 'type' =>  'error', 'msg' => 'No Event ID was supplied.', 'rows' => FALSE );
		}
		
		global $wpdb;
		$SQL = "SELECT question_groups, event_meta FROM ".$this->table_name." WHERE id = '%d'";
...
<pre>
	...
	$action_results = $events_detail_model->insert ($set_column_values);
</pre>
';
			die();
		}
...
<pre>
	...
	// model function to perform error checking and then run update
	$action_results = $events_detail_model->update ($set_column_values, $where);
</pre>
';
			die();
		}
...
// End of file Events_detail.model.php
// Location: /ee-mvc/models/Events_detail.model.php
?>