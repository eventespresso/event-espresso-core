 ...
 * ------------------------------------------------------------------------
 */
class Events_answer extends espresso_model {
	
	protected $table_name = EVENTS_ANSWER_TABLE;

	// array representation of wp_events_answer table and the data types for each field 
...
/**
 *		This function returns list of event relevant answers from the wp_events_answer table
 *
 *		example: 
 *
 ...
 *		@return array
 */	
	public function get_attendee_question_ids ( $attendee_id = FALSE ) {

		// what?? no attendee_id ??? Get outta here!!!
		if ( ! $attendee_id ) {
			return array( 'type' =>  'error', 'msg' => 'No ID was supplied.', 'rows' => FALSE );
		}
		
		$SQL = 'SELECT question_id FROM '.$this->table_name." WHERE attendee_id = '" . $attendee_id . "'";
 ...
 */	
	public function insert ($set_column_values) {
		...
		if ( $results['rows'] == 1 ) {
			...
			$results['msg'] = 'Answer details have been successfully saved to the database.';
		} elseif ( $results['rows'] > 1 ) {
			// multiple rows were successfully updated
			$results['msg'] = 'Details for '.$results.' answers have been successfully saved to the database.';
		} else {
			// error message 
			$results['msg'] = 'An error occured and the answer details have not been saved to the database.';
		}
 ...
 */	
	public function update ($set_column_values, $where) {
		...
		if ( $results['rows'] == 1 ) {
			// one row was successfully updated
			$results['msg'] = 'Answer details have been successfully updated.';
		} elseif ( $results['rows'] > 1 ) {
			// multiple rows were successfully updated
			$results['msg'] = 'Details for '.$results.' answers have been successfully updated.';
		} else {
			// error message 
			$results['msg'] = 'An error occured and the answer details have not been updated.';
		}
...
<pre>
	...
	$action_results = $answer_model->insert ($set_column_values);
</pre>
';
			die();
		}
...
<pre>
	...
	// model function to perform error checking and then run update
	$action_results = $answer_model->update ($set_column_values, $where);
</pre>
';
			die();
		}
...
// End of file Events_answer.model.php
// Location: /ee-mvc/models/Events_answer.model.php