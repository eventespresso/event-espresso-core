 ...
 * ------------------------------------------------------------------------
 *
 * Events Seating Chart Event Seat Model
 * 
 *
 * @package				Event Espresso
 * @subpackage		mvc-models
 * @author					Brent Christensen 
 * @table 						wp_events_seating_chart_event_seat
 *
 * ------------------------------------------------------------------------
 */
class Events_seating_chart_event_seat extends espresso_model {

	protected $table_name = EVENTS_SEATING_CHART_EVENT_SEAT_TABLE;
	...
	protected $table_data_types = array (	 
			...
			'event_id' 								=> '%d', 	
			'attendee_id' 						=> '%d', 	
			'purchase_price' 				=> '%f', 	// floats not recommended for money 
			'purchase_datetime' 	=> '%s', 	
			'by_admin' 							=> '%d', 	
			'occupied' 								=> '%d'
		);
 ...
 *		@return array
 */	
	public function delete_event_seat ( $attendee_id = FALSE ) {
		...
		global $wpdb;
		$SQL = 'DELETE FROM ' . $this->table_name . " WHERE attendee_id ='%d'";
		$result = $wpdb->get_row( $wpdb->prepare( $SQL, $attendee_id ));
 ...
 */	
	public function insert ($set_column_values) {
		...
		global $wpdb;
		// throw all vars into an array to make escaping them easier.
		$vars = array( 'set_column_values' => $set_column_values );
		foreach ( $vars as $varname => $var ) {
			$$varname = $wpdb->escape( $var );		//		yes the double $$ is correct
		}
 ...
 */	
	public function update ($set_column_values, $where) {
		...
		global $wpdb;
		// throw all vars into an array to make escaping them easier.
		$vars = array( 'set_column_values' => $set_column_values, 'where' => $where );
		foreach ( $vars as $varname => $var ) {
			$$varname = $wpdb->escape( $var );		//		yes the double $$ is correct
		}
...
// End of file Events_seating_chart_event_seat.model.php
// Location: /ee-mvc/models/Events_seating_chart_event_seat.model.php