...
abstract class EEM_TempBase extends EEM_Base{ 
	...
	*/	
	...
	public function ensure_is_obj($base_class_obj_or_id){ 
		...
		}elseif(intval($base_class_obj_or_id)){
			return $this->get_one_by_ID(intval($base_class_obj_or_id));
		}elseif(is_null($base_class_obj_or_id)){
			return null;
		}else{
			throw new EE_Error(sprintf(__("'%s' is neither an object of type %s, nor an ID!",'event_espresso'),$base_class_obj_or_id,$this->_getClasssName()));
		}
	}
