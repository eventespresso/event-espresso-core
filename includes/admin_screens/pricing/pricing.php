...
function espresso_price_manager_menu() {
	...
	if ( $_REQUEST['action'] != 'edit' && $_REQUEST['action'] != 'add_new_price' && $_REQUEST['action'] != 'add_new_type' ) {
		$edit_or_add_page = FALSE;
	} else {
		$edit_or_add_page = TRUE;
	}
...
//						echo printr( $PRT->type, '$PRT->type' );		
							foreach ($prices as $price ) {
									?>
									<tr>
										<td class="check-column" style="padding:7px 0 22px 5px; vertical-align:top;">
										...
										<td class="post-title page-title column-title">
											<strong><a href="admin.php?page=event_prices&action=edit_price&id=<?php echo $price->ID(); ?>"><?php echo stripslashes_deep($price->name()); ?></a></strong>
											
											<div class="row-actions">
												<span class="edit">
													<a href="admin.php?page=event_prices&action=edit_price&id=<?php echo $price->ID(); ?>"><?php _e('Edit', 'event_espresso'); ?></a>
												</span>
												 | 
												<span class="delete">
													<a onclick="return confirmDelete();" class="submitdelete" href="admin.php?page=event_prices&action=delete_price&id=<?php echo $price->ID(); ?>">
