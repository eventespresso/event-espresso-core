...
// check if class object is instantiated
		if (self::$_instance === NULL or !is_object(self::$_instance) or ! ( self::$_instance instanceof EE_Single_Page_Checkout )) {
...
//		printr( $this->cart->whats_in_the_cart(), 'whats_in_the_cart  <br /><span style="font-size:10px;font-weight:normal;">' . __FILE__ . '<br />line no: ' . __LINE__ . '</span>', 'auto' );
		...
		$template_args['success_msg_class'] = empty($notices['success']) ? ' ui-helper-hidden hidden' : ' fade-away';
		// error messages - none right?  : )
		$template_args['error_msg'] = empty($notices['errors']) ? '' : $notices['errors'];
		$template_args['error_msg_class'] = empty($notices['errors']) ? ' ui-helper-hidden hidden' : ' fade-away';
