...
//skip the first insert because we do not have the start dates
				...
				<div id="message" class="updated fade">
					<p>
						<strong><?php _e('The event', 'event_espresso'); ?>
							...
							<?php _e('has been added for ', 'event_espresso'); ?><?php echo date("m/d/Y", strtotime($start_date)); ?> <a href="admin.php?page=events&action=edit&event_id=<?php echo $last_event_id; ?>"><?php _e('Edit this event?', 'event_espresso'); ?>
							</a>
						</strong>
					</p>
				</div>
			<?php } else { ?>
				<div id="message" class="error">
					<p>
						<strong>
							<?php _e('There was an error in your submission, please try again. The event was not saved!', 'event_espresso'); ?><?php print $wpdb->print_error(); ?>.
						</strong>
					</p>
				</div>
				<?php
...
function cost() {
	...
	if (!empty($_REQUEST['event_cost'])) {
		...
		$sql_price = "INSERT INTO " . EVENTS_PRICES_TABLE . " (event_cost, surcharge, price_type) VALUES ('" . $event->id . "', '0.00', '0.00', '" . __('Free', 'event_espresso') . "', '0.00', '" . __('Free', 'event_espresso') . "')";
		if (!$wpdb->query($sql_price)) {
			$event->error = true;
		}
	}
	...
	if ($_REQUEST['create_post'] == 'Y') {
		...
		if ($post_type == 'post') {
			if (file_exists(EVENT_ESPRESSO_TEMPLATE_DIR . "event_post.php") || file_exists(EVENT_ESPRESSO_PLUGINFULLPATH . "templates/event_post.php")) {
				...
				ob_start();
				espresso_require_template("event_post.php");
		...
		add_post_meta($post_id, 'event_identifier', $event->event_identifier);
		add_post_meta($post_id, 'slug', $event->event_slug);
		add_post_meta($post_id, 'event_start_date', $_REQUEST['start_date']);
		add_post_meta($post_id, 'event_end_date', $_REQUEST['end_date']);
		add_post_meta($post_id, 'event_location', $event->event_location);
		add_post_meta($post_id, 'virtual_url', $event->virtual_url);
		add_post_meta($post_id, 'virtual_phone', $event->virtual_phone);
		add_post_meta($post_id, 'event_address', $event->address);
		add_post_meta($post_id, 'event_address2', $event->address2);
		add_post_meta($post_id, 'event_city', $event->city);
		add_post_meta($post_id, 'event_state', $event->state);
		add_post_meta($post_id, 'event_country', $event->country);
		add_post_meta($post_id, 'event_phone', $event->phone);
		add_post_meta($post_id, 'venue_title', $event->venue_title);
		add_post_meta($post_id, 'venue_url', $event->venue_url);
		add_post_meta($post_id, 'venue_phone', $event->venue_phone);
		add_post_meta($post_id, 'venue_image', $event->venue_image);
		add_post_meta($post_id, 'event_externalURL', $event->externalURL);
		add_post_meta($post_id, 'event_reg_limit', $event->reg_limit);
		add_post_meta($post_id, 'event_start_time', time_to_24hr($event->start_time));
		add_post_meta($post_id, 'event_end_time', time_to_24hr($event->end_time));
		add_post_meta($post_id, 'event_registration_start', $event->registration_start);
		add_post_meta($post_id, 'event_registration_end', $event->registration_end);
		add_post_meta($post_id, 'event_registration_startT', $event->registration_startT);
		add_post_meta($post_id, 'event_registration_endT', $event->registration_endT);
