...
function espresso_send_to_firstdata_connect_2($EE_Session) {
	...
	$fdggutil->set_returnUrl( get_permalink( EE_Registry::instance()->CFG->core->txn_page_id ));
	$fdggutil->set_cancelUrl( get_permalink( EE_Registry::instance()->CFG->core->cancel_page_id ));
...
function espresso_process_firstdata_connect_2($EE_Session) {
	...
	if($_REQUEST['status']=='APPROVED') {
		...
		include("lib/Fdggutil.php");
		$fdggutil = new espresso_Fdggutil( $firstdata_connect_2_settings['storename'], $firstdata_connect_2_settings['sharedSecret'] );
