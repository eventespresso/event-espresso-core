<?php
//This file is used for Event Espresso 3.2.*
...
function espresso_add_calendar_to_admin_menu($submenu_page_sections, $espresso_manager) {
	global $espresso_premium;
	$submenu_page_sections['calendar'] = array(
			($espresso_premium),
			'events',
...
function espresso_add_calendar_meta_boxes() {
	global $espresso_premium;
	$screen = get_current_screen();
	$screen_id = $screen->id;
	if ($screen_id == 'event-espresso_page_espresso_calendar') {
		...
		add_meta_box('espresso_calendar_usage_metabox', __('Calendar Usage', 'event_espresso'), 'espresso_calendar_usage_metabox', $screen_id, 'normal');
		add_meta_box('espresso_calendar_basic_settings_metabox', __('Basic Settings', 'event_espresso'), 'espresso_calendar_basic_settings_metabox', $screen_id, 'normal');
		add_meta_box('espresso_calendar_advanced_settings_metabox', __('Advanced Settings', 'event_espresso'), 'espresso_calendar_advanced_settings_metabox', $screen_id, 'normal');
...
function espresso_calendar_advanced_settings_metabox($espresso_calendar) {
	?>
	<div class="padding">
...
function espresso_calendar_basic_settings_metabox($espresso_calendar) {
	$values = array(
			array('id' => false, 'text' => __('No', 'event_espresso')),
			array('id' => true, 'text' => __('Yes', 'event_espresso'))
	);
...
function espresso_calendar_usage_metabox($espresso_calendar) {
	?>
	<div class="padding">
		<ul>
			<li> <strong>
